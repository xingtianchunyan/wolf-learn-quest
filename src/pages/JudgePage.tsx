...
const JudgePage = () => {
  ...
  } = useRoleSelection(roomId || '', currentUser?.id || null, players.length, 8);
  if (!roomId) {
    return <PageLayout>
        ...
        <div className="container mx-auto py-6 px-4 min-h-[calc(100vh-4rem)]">
          ...
          height: 'calc(100vh - 16rem)'
            ...
            <div className="lg:col-span-6 flex flex-col gap-6 h-full">
              ...
              {/* 添加技能系统管理器 */}
              {gameState && <div className="h-1/3">
                  <SkillSystemManager roomId={roomId} gameStateId={gameState.id} userId={currentUser?.id || ''} isJudge={true} currentPhase={gameState.currentPhase} roleState={null} roleDesign={null} players={playersWithRoles} />
                </div>}
            </div>
