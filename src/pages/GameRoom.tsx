...
const fetchData = async () =>  {
      try {
        ...
        if (id) { console.log('Fetching room data for room ID:', id);
          ...
          if (roomPlayerData?.rooms) { const room = roomPlayerData.rooms;
...
const handleReadyToggle = async () =>  { if (!currentUser || !roomData || !currentPlayerRecord) return;
    ...
    try { const success = await updatePlayerReady(currentPlayerRecord.id, newReadyState);
