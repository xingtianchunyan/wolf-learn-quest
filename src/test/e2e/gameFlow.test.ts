...
import { createGameScenario, createMockRoom, createMockGameState, createMockSkillUse } from '@/test/helpers/testFactory'
...
describe('游戏流程 E2E 测试', () => {
  ...
  describe('完整游戏流程', () => {
    it('应该完成一个完整的游戏回合', async () => {
      ...
      const roomBuilder = createMockQueryBuilder({ data: mockRoom, error: null })
      ...
      // 2. 初始化游戏状态
      const mockGameState = createMockGameState({ id: testGameStateId, room_id: testRoomId })
      const gameStateBuilder = createMockQueryBuilder({ data: mockGameState, error: null })
      ...
      const { data: attackResult, error: attackError } = await supabase.rpc('use_skill_enhanced', {
      ...
      const { data: conflicts, error: conflictError } = await supabase.rpc('detect_skill_conflicts', {
        p_game_state_id: testGameStateId,
        p_phase: 'night',
        p_round_number: 1
      })
    ...
    it('应该正确处理多重保护导致的淘汰', async () => {
      ...
      const { data: multiProtect, error: protectError } = await supabase.rpc('check_multiple_protection', {
        p_game_state_id: testGameStateId,
        p_round_number: 1,
