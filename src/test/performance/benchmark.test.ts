...
// 添加 getInstance 方法
mockPerformanceCriticalFixes.getInstance = vi.fn().mockReturnValue(mockPerformanceCriticalFixes);
...
class PerformanceBenchmark {
  ...
  async benchmarkOperation(
    ...
    for (let i = 0; i < config.testRuns; i++) {
      ...
      if (currentMemory - initialMemory > config.memoryLimit) {
        errors.push(`Memory limit exceeded: ${currentMemory - initialMemory} bytes`);
        break;
      }
    }

    const finalMemory = this.getMemoryUsage();
    const result = this.calculateMetrics(operation.name, times, successCount, config.testRuns, errors, finalMemory - initialMemory);
  ...
  async loadTest(
    ...
    for (let i = 0; i < config.concurrency; i++) {
      const promise = operation.operation()
        ...
        .catch(error => {
          errors.push(`Concurrent operation ${i + 1}: ${error instanceof Error ? error.message : String(error)}`);
        });
    ...
    for (let load = 1; load <= config.concurrency; load++) {
      ...
      for (let i = 0; i < load; i++) {
        const promise = operation.operation()
          ...
          .catch(error => {
            errors.push(`Stress test load ${load}, operation ${i + 1}: ${error instanceof Error ? error.message : String(error)}`);
          });
  ...
  generateReport(): string {
    ...
    this.results.forEach(result => {
      ...
      report.push(`平均时间: ${result.averageTime.toFixed(2)}ms`);
      report.push(`最小时间: ${result.minTime.toFixed(2)}ms`);
      report.push(`最大时间: ${result.maxTime.toFixed(2)}ms`);
      report.push(`标准差: ${result.standardDeviation.toFixed(2)}ms`);
      report.push(`每秒操作数: ${result.operationsPerSecond.toFixed(2)}`);
      report.push(`内存使用: ${(result.memoryUsage / 1024 / 1024).toFixed(2)}MB`);
...
describe('性能基准测试套件', () => {
  let benchmark: PerformanceBenchmark = new PerformanceBenchmark();
