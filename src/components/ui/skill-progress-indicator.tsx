...
export interface SkillProgressIndicatorProps {
  skillName: string;
...
export const SkillProgressIndicator: React.FC<SkillProgressIndicatorProps> = ({
  skillName,
  skillChineseName,
  steps,
  currentStep,
  ...
  return (
    <TooltipProvider>
      <Card className={`border-l-4 ${
        overallStatus === 'completed' ? 'border-l-green-500' :
        overallStatus === 'processing' ? 'border-l-blue-500' :
        overallStatus === 'failed' ? 'border-l-red-500' :
        'border-l-gray-300'
      } ${className}`}>
                  ...
                  <Tooltip key={step.id}>
                    ...
                    <TooltipContent>
                      <div className="text-sm">
                        ...
                        {step.duration && <div>耗时: {step.duration}ms</div>}
                        {step.errorMessage && (
                          <div className="text-red-600">错误: {step.errorMessage}</div>
                        )}
                      </div>
                    </TooltipContent>
                  </Tooltip>
...
// 预设的技能执行步骤模板
export const SKILL_EXECUTION_STEPS = {
  werewolf_attack: [
    { id: 'validation', name: '验证攻击条件' },
