...
* @since 2024-12-19
* @category judge
* @filepath judge\management\QuestionBankPanel.tsx
 */
...
/**
* QuestionBankPanel 组件
*
* 提供裁判功能和游戏管理
*
* @component
* @param { QuestionBankPanelProps } props - 组件属性
* @returns { JSX.Element } 渲染的组件
* @hooks useState, useRef, useEffect, useToast
*
* @example
* // 使用示例
...
const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) =>  { const file = event.target.files?.[0];
    ...
    if (!allowedFormats.includes(fileExtension)) { const errorMsg = '请上传 TXT、DOC、DOCX、XLS、XLSX、PPTX 或 MD 格式的文件';
      setError(errorMsg);
      toast({
        title: '文件格式不支持',
    ...
    if (existingFile) { const errorMsg = '该文件已经上传过了';
      setError(errorMsg);
      toast({
        title: '文件重复',
        description: errorMsg,
    ...
    try { const originalName = file.name;
...
const handleGenerateQuestions = async () => { if (!selectedPreprocessedFile)  {
    ...
    try { console.log('调用生成题目API:', { selectedPreprocessedFile  });
