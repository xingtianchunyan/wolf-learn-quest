...
const PreparationPhaseDialog: React.FC<PreparationPhaseDialogProps> = ({
  ...
  const isPlayerOnline = (player: any) => {
    return onlinePlayersList.some(onlinePlayer => onlinePlayer.user_id === player.id || player.name && player.name.includes(onlinePlayer.user_id));
  };
  if (!isOpen) return null;
  return <div className="fixed inset-0 z-50 pointer-events-none">
      <div ref={dialogRef} className="absolute pointer-events-auto bg-werewolf-card border-werewolf-purple/30 border rounded-lg shadow-xl" style={{
      ...
      height: '600px'
        ...
        <div className="dialog-header p-4 cursor-move border-b border-werewolf-purple/30">
          ...
          <button onClick={onClose} className="absolute right-4 top-4 rounded-sm opacity-70 hover:opacity-100 text-werewolf-purple">
            ✕
          </button>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 p-4 h-[calc(100%-80px)]">
          ...
          <div className="col-span-4 flex flex-col gap-4">
            ...
            {/* 开始游戏按钮 */}
            <div className="flex-shrink-0">
              <Button onClick={handleStartGame} disabled={!allPlayersReady || players.length === 0} className={`w-full px-4 py-3 text-lg ${allPlayersReady && players.length > 0 ? 'bg-werewolf-purple hover:bg-werewolf-light' : 'bg-gray-600 cursor-not-allowed'}`}>
                ...
                <CardContent className="p-4 pt-0 h-[calc(100%-80px)]">
                  <ScrollArea className="h-full">
                    <div className="space-y-3 pr-4">
                      {questions.map(question => <div key={question.id} className={`p-3 rounded-md border cursor-pointer transition-colors ${selectedQuestionId === question.id ? 'border-werewolf-purple bg-werewolf-purple/20' : 'border-werewolf-purple/30 hover:border-werewolf-purple/50'}`} onClick={() => setSelectedQuestionId(question.id)}>
                  ...
                  <ScrollArea className="h-full">
                    <div className="border border-werewolf-purple/30 rounded-md">
                      <Table>
                        ...
                        <TableBody>
                          {playersLoading ? <TableRow>
                              <TableCell colSpan={5} className="text-center text-gray-400 py-4">
                                加载中...
                              </TableCell>
                            </TableRow> : players.length === 0 ? <TableRow>
                              <TableCell colSpan={5} className="text-center text-gray-400 py-4">
                                暂无玩家
                              </TableCell>
                            </TableRow> : players.map(player => <TableRow key={player.id} className="border-werewolf-purple/30">
                                ...
                                <TableCell>
                                  <div className="flex items-center space-x-2">
                                    {isPlayerOnline(player) ? <Wifi className="h-4 w-4 text-green-400" /> : <WifiOff className="h-4 w-4 text-red-400" />}
                                ...
                                <TableCell>
                                  <div className="flex items-center space-x-2">
                                    {player.isReady ? <UserCheck className="h-4 w-4 text-green-400" /> : <UserX className="h-4 w-4 text-red-400" />}
