...
export const ErrorDisplayComponent: React.FC<ErrorDisplayProps> = ({
  ...
  const [feedbackType, setFeedbackType] = useState<'helpful' | 'not_helpful' | 'suggestion' | 'bug_report'>('helpful');
  const [executingSolution, setExecutingSolution] = useState<string | null>(null);
  const [copiedToClipboard, setCopiedToClipboard] = useState(false);

  const { collectFeedback, updateSolutionSuccess } = useEnhancedUserErrorInterface(config);
   ...
   * 执行选定的解决方案
   */
  const handleSolutionExecute = useCallback(async (solution: ErrorSolution) => {
...
时间: ${new Date().toLocaleString()}
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 ${className}`}>
      <div className={`max-w-2xl w-full max-h-[90vh] overflow-y-auto rounded-lg border-2 ${styles.container} shadow-xl`}>
        {/* 头部 */}
                ...
                {errorInfo.solutions.map((solution, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      {(solution.action || onSolutionExecute) && solution.type === 'automatic' && (
                        <button
