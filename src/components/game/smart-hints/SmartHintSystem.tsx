...
const SmartHintSystem: React.FC<SmartHintSystemProps> = ({
  ...
  const smartHints = useMemo(() => {
    ...
    if (gameState.phase === 'night' && gameState.userRole === 'vigil') {
      const vigilSkill = gameState.availableSkills.find(s => s.name === 'vigil');
    ...
    if (gameState.phase === 'night' && gameState.userRole === 'witch') {
      const potionSkill = gameState.availableSkills.find(s => s.name === 'magic_potion');
    ...
    if (gameState.round >= 3) {
      const deadPlayers = gameState.players.filter(p => p.status === 'dead').length;
