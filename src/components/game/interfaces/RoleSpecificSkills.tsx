...
export const RoleSpecificSkills: React.FC<RoleSpecificSkillsProps> = ({
  ...
  const SeerSkill = () => {
    // 获取预言家的查验记录
    const seerInvestigations = fullSkillUses.filter(
      use =>
        use.user_id === userId &&
        (use.skill_name === 'prophecy' ||
          use.skill_name === 'seer_investigation') &&
  ...
  const renderRoleSkill = () => {
    ...
    if (
      roleNameLower?.includes('werewolf') ||
      roleNameLower?.includes('狼人')
    ) {
  ...
  return <div className='space-y-4'>{renderRoleSkill()}</div>;
};
