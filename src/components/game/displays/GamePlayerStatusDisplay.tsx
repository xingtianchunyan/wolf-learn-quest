...
const GamePlayerStatusDisplay: React.FC<GamePlayerStatusDisplayProps> = ({
  ...
  // 获取当前用户的角色信息
  const currentUserRoleState = currentUser
    ? roleStates.find(rs => rs.user_id === currentUser.id)
    : null;
  const _currentUserRoleSelection = currentUser
    ? getSelectedRoleByUser(currentUser.id)
    : null;
  ...
  // 使用统一的角色工具判断角色类型
  const _isCurrentUserWerewolf = canAccessWerewolfChannel(
    currentUserRole,
    currentUserRoleDesign
  );
  ...
  const getPlayerCardClass = (player: (typeof displayPlayers)[0]) => {
    ...
    const isNightPhase = currentPhase === 3 || currentPhase === 4;
    const isSelectable =
      canSelectTargets && (player.userId !== currentUser?.id || isNightPhase);
    const isEliminated =
      roleStates.find(rs => rs.user_id === player.userId)?.role_status === 4;

    return `${baseClass} bg-werewolf-dark/40 border-2 ${statusBorder} ${
      isSelected
        ? 'ring-2 ring-werewolf-purple ring-offset-2 ring-offset-werewolf-dark'
        : ''
    } ${
      isSelectable && !isEliminated
        ? 'hover:bg-werewolf-dark/60 cursor-pointer'
        : ''
    ...
    <div className='space-y-4'>
      ...
      <div className='grid grid-cols-4 gap-3 auto-rows-max'>
        {displayPlayers.map(player => {
          const { roleName, roleImageUrl, showRole } =
