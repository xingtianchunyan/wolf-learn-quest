...
const SkillConflictVisualization: React.FC<SkillConflictVisualizationProps> = ( { conflicts,
  onResolveConflict,
  gamePhase,
  isJudge }) => { const [resolutionProgress, setResolutionProgress] = useState<Record<string, number>>({ });
...
/**
 * getConflictDescription函数
 * 获取数据
 *
 * @param conflict - conflict参数
 * @returns void
 */
  ...
  return (;
    ...
    { conflicts.map(conflict => (;
      ...
      { conflict.conflictingSkills.map((skill, index) => (;
        ...
        <Badge variant='outline'>优先级: { skill.priority 
}</Badge>;
        </div>
      ...
      >
      { selectedConflict === conflict.id ? '隐藏' : '显示' 
}详细信息;
      </Button>
      ...
      { selectedConflict === conflict.id && (;
        ...
        >
        <div>冲突ID: { conflict.id 
}</div>
        <div>检测时间: { new Date().toLocaleTimeString() 
}</div>
        <div>游戏阶段: { gamePhase 
}</div>
        <div>
