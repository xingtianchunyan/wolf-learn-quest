...
const getEffectIcon = useCallback((effectType: string) => { const cacheKey = `effect_icon_$ {effectType 
    ...
    switch (effectType) { case 'elimination': icon = <Skull className='w-4 h-4' />; break;
...
useEffect(() =>  { if (!performanceConfig.enableMonitoring) return;
    ...
    const memoryMonitor = setInterval(() => { if (performance.memory) {
        ...
        // 内存阈值检查
        if (memoryUsage > performanceConfig.memoryThreshold) { logger.warn('内存使用过高，执行清理', { memoryUsage  });
