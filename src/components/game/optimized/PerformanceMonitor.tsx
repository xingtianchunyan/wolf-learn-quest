...
const PerformanceMonitor = memo<PerformanceMonitorProps>(({
  ...
  const analyzePerformance = useCallback(() => {
    ...
    // 内存使用检查
    if (performanceState.memoryUsage > thresholds.memoryUsage) {
      newWarnings.push({
    ...
    // 缓存命中率检查
    if (performanceState.cacheHitRate < thresholds.cacheHitRate) {
      newWarnings.push({
    ...
    // 订阅数量检查
    if (performanceState.subscriptionCount > 20) {
      newWarnings.push({
  ...
  const generateOptimizationSuggestions = useCallback(() => {
    ...
    // 内存优化建议
    if (performanceState.memoryUsage > 30 * 1024 * 1024) {
      suggestions.push({
    ...
    // 缓存优化建议
    if (performanceState.cacheHitRate < 0.7) {
      suggestions.push({
    ...
    // 订阅优化建议
    if (performanceState.subscriptionCount > 15) {
      suggestions.push({
  ...
  const getPerformanceGrade = useMemo(() => {
    ...
    if (memoryMB > thresholdMB) {
      score -= Math.min(25, (memoryMB - thresholdMB) * 2);
    }

    // 缓存命中率评分
    if (performanceState.cacheHitRate < thresholds.cacheHitRate) {
      score -= (thresholds.cacheHitRate - performanceState.cacheHitRate) * 30;
    }

    // 订阅数量评分
    if (performanceState.subscriptionCount > 10) {
      score -= Math.min(15, (performanceState.subscriptionCount - 10) * 1.5);
    }
