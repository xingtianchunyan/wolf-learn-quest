...
const SkillEffectsVirtualList = memo<SkillEffectsVirtualListProps>(({ effects,
  ...
  // 性能监控状态
  const [performanceState, setPerformanceState] = useState<PerformanceMonitoringState>({ renderCount: 0,
    averageRenderTime: 0,
    scrollEventCount: 0,
    memoryUsage: 0,
  ...
  * 基于滚动位置和容器高度计算需要渲染的项目范围
  ...
  const calculateVisibleRange = useCallback((scrollTop: number, effects: SkillEffect[]) => { const { itemHeights  } = virtualizationState;
    ...
    // 使用缓存的高度计算起始索引
    for (let i = 0; i < effects.length; i++) { const cachedHeight = itemHeights.get(effects[i].id) || itemHeight;
    ...
    for (let i = startIndex; i < effects.length; i++) { const cachedHeight = itemHeights.get(effects[i].id) || itemHeight;
  ...
  * 优化的滚动处理，包含防抖和性能监控
  ...
  const handleScroll = useCallback((event: React.UIEvent<HTMLDivElement>) => { const scrollTop = event.currentTarget.scrollTop;
    ...
    scrollTimeout.current = setTimeout(() => { const { startIndex, endIndex  } = calculateVisibleRange(scrollTop, effects);
      ...
      setVirtualizationState(prev => ({ ...prev,
        scrollTop,
        startIndex,
        endIndex,
        visibleItems,
