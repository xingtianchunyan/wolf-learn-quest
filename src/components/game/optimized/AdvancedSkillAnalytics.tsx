 ...
 * 接口注释：高级技能分析属性
 */
export interface AdvancedSkillAnalyticsProps {
  /** 技能数据 */
  skillData: SkillData;
  /** 技能统计 */
  stats: SkillStats;
 ...
 * - 优化建议提供
 */
const AdvancedSkillAnalytics = memo<AdvancedSkillAnalyticsProps>(({
  skillData,
  stats,
  performanceState,
  timeRange = 24,
  ...
  const timeSlotStats = useMemo((): TimeSlotStats[] => {
    const slots: { [hour: number]: { uses: number; successes: number; totalDuration: number } } = {};
    
    // 初始化24小时时间段
    for (let i = 0; i < 24; i++) {
      slots[i] = { uses: 0, successes: 0, totalDuration: 0 };
    }
    
    // 统计每小时的使用情况
    filteredData.uses.forEach(use => {
      const hour = new Date(use.timestamp).getHours();
      slots[hour].uses++;
      if (use.success) {
        slots[hour].successes++;
      }
      if (use.duration) {
        slots[hour].totalDuration += use.duration;
      }
    });
    
    // 转换为数组格式
    return Object.entries(slots).map(([hour, data]) => ({
      hour: parseInt(hour),
      uses: data.uses,
  ...
  const effectTypeStats = useMemo((): EffectTypeStats[] => {
    const typeMap: { [type: string]: { count: number; successes: number; totalDuration: number } } = {};
    
    // 统计当前活跃效果
    filteredData.targets.forEach(target => {
      if (!typeMap[target.effectType]) {
        typeMap[target.effectType] = { count: 0, successes: 0, totalDuration: 0 };
      }
      typeMap[target.effectType].count++;
      if (target.isActive) {
        typeMap[target.effectType].successes++;
      }
   ...
   * 函数级注释：生成分析结果
   */
  const generateAnalysisResult = useCallback((): AnalysisResult => {
    ...
    if (performanceState.averageRenderTime > 16) {
      performanceImpact = 'high';
      recommendations.push('优化渲染性能，减少重复渲染');
    } else if (performanceState.averageRenderTime > 8) {
      performanceImpact = 'medium';
      recommendations.push('考虑使用 React.memo 优化组件');
    }
    
    if (performanceState.memoryUsage > 50 * 1024 * 1024) {
      recommendations.push('清理未使用的缓存和订阅');
    }
    
    if (successRate < 80) {
      recommendations.push('检查技能使用条件和时机');
    }
    
    if (usageFrequency > 5) {
      recommendations.push('考虑合并相似的技能操作');
    }
   ...
   * 函数级注释：获取效果类型颜色
   */
  const getEffectTypeColor = useCallback((type: string) => {
    const colors = {
        ...
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            <Badge variant="outline" className={
              analysisResult.efficiency.grade === 'A' ? 'border-green-500 text-green-500' :
          ...
          <TabsContent value="performance" className="space-y-4">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={
                  analysisResult.performance.impact === 'high' ? 'destructive' :
