      ...
      <CardContent>
        <div className='space-y-3'>
          {roleStates.map(roleState => {
            ...
            const skillRemaining = getSkillUsesRemaining(
              roleState.skill_uses_remaining
            );
            const isHunterRevengeState = isHunterRevenge(
              roleState.status_effects
            );
            const revengeTimeLeft = getHunterRevengeTimeLeft(
              roleState.status_effects
            );
            ...
            return (
              <Collapsible
                key={roleState.id}
                open={isExpanded}
                onOpenChange={() => togglePlayerExpansion(roleState.id)}
              >
                <div
