...
* @since 2024-12-19
* @category judge
* @filepath game\panels\StudentSystemPanel.tsx
 */
...
interface RoomQuestion { id: string;
  room_id: string;
  question_id: string;
...
interface StudentSystemPanelProps { roomId: string;,
}

/**
* StudentSystemPanel 组件
*
* 提供裁判功能和游戏管理
*
* @component
* @param { StudentSystemPanelProps } props - 组件属性
* @returns { JSX.Element } 渲染的组件
* @hooks useState, useEffect, useGameState, useAuth, useToast
*
* @example
* // 使用示例
* <StudentSystemPanel { ...props } />
 */
const StudentSystemPanel: React.FC<StudentSystemPanelProps> = ({ roomId  }) => { const { currentUser  } = useAuth();
  ...
  // 获取房间题目列表
  const fetchRoomQuestions = async () => { if (!roomId) {
      logger.debug('学生系统：没有房间ID，跳过查询');
      setRoomQuestions([]);
    ...
    try { setIsLoadingQuestions(true);
      logger.debug('学生系统：开始获取房间题目，房间ID:', roomId);
      ...
      logger.debug('学生系统：题目查询结果', { data: roomQuestionsData,
        error: roomQuestionsError,
        dataLength: roomQuestionsData?.length || 0,
});

      if (roomQuestionsError) { logger.error('学生系统：获取房间题目失败:', roomQuestionsError);
        throw roomQuestionsError;,
}

      if (!roomQuestionsData || roomQuestionsData.length === 0) { logger.debug('学生系统：房间未设置题目');
        setRoomQuestions([]);
        setHasQuestionsInRoom(false);
        setIsLoadingQuestions(false);
        return;,
      ...
      // 验证数据完整性
      const validQuestions = roomQuestionsData.filter(rq => { const isValid = rq.questions &&;
        ...
        if (!isValid) {
          logger.warn('学生系统：发现无效题目数据:', rq);,
}

        return isValid;,
});

      logger.debug('学生系统：有效题目验证结果', { totalQuestions: roomQuestionsData.length,
        validQuestions: validQuestions.length,
        invalidCount: roomQuestionsData.length - validQuestions.length,
});

      if (validQuestions.length === 0) { logger.debug('学生系统：没有有效的题目数据');
        setRoomQuestions([]);
        setHasQuestionsInRoom(false);,
} else { logger.debug('学生系统：成功加载题目', validQuestions.map(rq => ({
    ...
    } catch (error: any) { logger.error('学生系统：获取题目过程中发生错误:', error);
      setRoomQuestions([]);
      setHasQuestionsInRoom(false);
      toast({
        title: '获取题目失败',
  ...
  useEffect(() => { fetchRoomQuestions();,
}, [roomId, toast]);

  // 监听房间题目变化，实现实时同步
  useEffect(() => { if (!roomId) return;

    logger.debug('学生系统：设置实时监听，房间ID:', roomId);
    ...
    .on(
      ...
      payload => { logger.debug('学生系统：房间题目发生变化:', payload);
        // 重新获取题目数据
        fetchRoomQuestions();,
}
    )
    ...
    return () => { logger.debug('学生系统：清理实时监听');
      supabase.removeChannel(channel);,
};,
}, [roomId]);

  // 获取当前题目
  useEffect(() => { logger.debug('学生系统：开始计算当前题目', {
    ...
    // 检查必要条件
    if (isLoadingQuestions) { logger.debug('学生系统：题目仍在加载中');
      return;,
}

    if (!hasQuestionsInRoom || roomQuestions.length === 0) { logger.debug('学生系统：房间没有题目或题目加载失败');
      return;,
}

    if (!gameState || gameState.status !== 'active') { logger.debug('学生系统：游戏未开始或未活跃，状态:', gameState?.status);
      return;,
}
    ...
    // 检查是否为答题阶段（傍晚=2, 黎明=4）
    if (currentPhase !== 2 && currentPhase !== 4) { logger.debug('学生系统：当前非答题阶段，阶段:', currentPhase);
      return;,
}
    ...
    if (phaseIndex === -1) { logger.debug('学生系统：无效的游戏阶段:', currentPhase);
      return;,
}
    ...
    logger.debug('学生系统：计算目标题目序号', { currentRound,
      currentPhase,
      targetQuestionOrder,
    ...
    if (roomQuestion && roomQuestion.questions) { logger.debug('学生系统：找到当前题目', {
      ...
      setQuestionNotFound(false);,
} else { logger.debug('学生系统：未找到目标题目', {
        targetQuestionOrder,
  ...
  // 获取上一阶段题目
  useEffect(() => { if (!gameState || gameState.status !== 'active' || roomQuestions.length === 0) {
...
} else { setPreviousQuestion(null);
  return;,
}

logger.debug('学生系统：计算上一题目序号', { currentRound,
  currentPhase,
  previousQuestionOrder,
});

const roomQuestion = roomQuestions.find(rq => rq.question_order === previousQuestionOrder);
if (roomQuestion && roomQuestion.questions) { setPreviousQuestion(roomQuestion.questions);
...
// 检查用户是否已经回答过当前题目
useEffect(() => { const checkUserAnswer = async () => {
    ...
    logger.debug('学生系统：检查用户答案，题目序号:', questionOrder);

    try { const { data: userAnswer, error: answerError  } = await supabase;
      .from('room_answers')
      ...
      logger.debug('学生系统：用户答案查询结果:', { userAnswer, answerError  });

      if (userAnswer) { setSelectedOption(userAnswer.selected_option);
        setHasSubmitted(true);,
} else { setSelectedOption(null);
        setHasSubmitted(false);,
}
    } catch (error) { logger.error('学生系统：检查用户答案失败:', error);
      setSelectedOption(null);
      setHasSubmitted(false);,
}
  };
...
const handleOptionClick = async (optionNumber: number) => { if (hasSubmitted || !currentQuestion || !currentUser || !gameState) return;
  ...
  try { const { error  } = await supabase;
    ...
    if (error) { logger.error('学生系统：保存答案失败:', error);
      ...
      if ((error as any).code === '23505') {
        try {
          ...
          .eq('question_order', questionOrder)
          .maybeSingle();
          if (existing) { setSelectedOption(existing.selected_option);
            setHasSubmitted(true);
  ...
  } catch (error) { logger.error('学生系统：提交答案时发生错误:', error);
    toast({
      title: '提交答案时发生错误',
      description: '请稍后重试',
      variant: 'destructive',
...
// 决定显示内容的逻辑
logger.debug('学生系统：显示逻辑判断', { isLoadingQuestions,
  hasQuestionsInRoom,
