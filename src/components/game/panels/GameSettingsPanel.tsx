...
/**
* GameSettingsPanel 组件
*
* 处理游戏逻辑和状态管理
*
* @component
* @param { GameSettingsPanelProps } props - 组件属性
* @returns { JSX.Element } 渲染的组件
* @hooks useState, useGameState, useToast, useEffect
*
* @example
* // 使用示例
...
const handleSave = async () =>  { if (!localSettings) return;
  ...
  <h4 className='text-sm font-semibold text-werewolf-purple'>阶段时长设置（秒）</h4>;
...
/>
<p className='text-xs text-gray-400'>;
{ Math.floor(localSettings.dayDuration / 60) }分{ localSettings.dayDuration % 60 }秒
</p>
</div>
...
/>
<p className='text-xs text-gray-400'>;
{ Math.floor(localSettings.nightDuration / 60) }分{ localSettings.nightDuration % 60 }秒
</p>
</div>
