...
const PlayerStatusManager: React.FC<PlayerStatusManagerProps> = ({
  ...
  const [recentChanges, setRecentChanges] = useState<
    Map<string, { fromStatus: number; toStatus: number; timestamp: number }>
  >(new Map());

  // 获取当前用户角色信息
  const currentUserRoleSelection = currentUser
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setRecentChanges(prevChanges => {
        ...
        return filteredChanges.size !== prevChanges.size
          ? filteredChanges
          : prevChanges;
      });
    }, 1000); // 每秒检查一次
  ...
  const getPlayerStatusInfo = (player: Player) => {
    ...
    const isRecentChange =
      recentChange && Date.now() - recentChange.timestamp < 5000;
  ...
  const getStatusChangeAnimation = (
    statusInfo: ReturnType<typeof getPlayerStatusInfo>
  ) => {
            ...
            >
              <div className='flex items-center gap-3'>
                ...
                <div className='relative'>
                  ...
                  {/* 状态指示器 */}
                  <div
                    className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-werewolf-dark ${
                      statusInfo.status === ROLE_STATUS.DYING
                        ? 'bg-red-500'
                        : statusInfo.status === ROLE_STATUS.WEAK
                          ? 'bg-yellow-500'
                          : statusInfo.status === ROLE_STATUS.ELIMINATED
                            ? 'bg-gray-500'
                            : 'bg-green-500'
                    }`}
                  />
