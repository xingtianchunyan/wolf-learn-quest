...
export const SkillSystemMonitor: React.FC<SkillSystemMonitorProps> = ({
  ...
  // 使用技能效果处理器
  const processor = useSkillEffectProcessor(gameStateId, {
    autoProcess: isJudge, // 只有法官才自动处理
    intervalMs: 3000,
    enableLogging: true
  });
  ...
  const evaluateSystemHealth = (
    ...
    // 数据库健康检查
    if (metrics.totalSkillUses > 1000) {
      health.database = 'warning';
    }

    // 处理器健康检查
    if (processorStats.failureCount > 10 || metrics.errorRate > 20) {
      health.processing = 'critical';
    } else if (processorStats.failureCount > 5 || metrics.errorRate > 10) {
      health.processing = 'warning';
    }

    // 队列健康检查
    if (metrics.queuedEffects > 50) {
      health.queue = 'critical';
    } else if (metrics.queuedEffects > 20) {
