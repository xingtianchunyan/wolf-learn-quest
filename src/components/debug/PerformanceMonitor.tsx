...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const getEventListenerCount = (): number => {
    ...
    elements.forEach(element => {
      // 检查常见的事件属性
      const eventAttributes = ['onclick', 'onmouseover', 'onkeydown', 'onchange'];
  ...
  const checkAlerts = (data: PerformanceData): string[] => {
    ...
    if (data.latency > defaultThresholds.latency.critical) {
      newAlerts.push(`网络延迟过高: ${data.latency.toFixed(1)}ms`);
    } else if (data.latency > defaultThresholds.latency.warning) {
      newAlerts.push(`网络延迟警告: ${data.latency.toFixed(1)}ms`);
    }

    if (data.renderTime > defaultThresholds.renderTime.critical) {
      newAlerts.push(`渲染时间过长: ${data.renderTime.toFixed(1)}ms`);
    } else if (data.renderTime > defaultThresholds.renderTime.warning) {
      newAlerts.push(`渲染时间警告: ${data.renderTime.toFixed(1)}ms`);
    }
   ...
   * 获取性能状态
   */
  const getPerformanceStatus = (value: number, thresholds: { warning: number; critical: number }, reverse = false): 'good' | 'warning' | 'critical' => {
    if (reverse) {
  ...
  const previousData = history.length > 1 ? history[history.length - 2] : currentData;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between text-sm">
              ...
              {getTrendIcon(currentData.memory.percentage, previousData.memory.percentage)}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <div className={`text-2xl font-bold ${getStatusColor(
                getPerformanceStatus(currentData.memory.percentage, defaultThresholds.memory)
              )}`}>
                {currentData.memory.percentage.toFixed(1)}%
              </div>
              <Progress value={currentData.memory.percentage} className="h-2" />
              <div className="text-xs text-muted-foreground">
                {(currentData.memory.used / 1024 / 1024).toFixed(1)}MB / {(currentData.memory.total / 1024 / 1024).toFixed(1)}MB
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className={`text-2xl font-bold ${getStatusColor(
                getPerformanceStatus(currentData.fps, defaultThresholds.fps, true)
              )}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className={`text-2xl font-bold ${getStatusColor(
                getPerformanceStatus(currentData.latency, defaultThresholds.latency)
              )}`}>
                {currentData.latency.toFixed(0)}ms
              </div>
              <Progress value={Math.min((currentData.latency / 200) * 100, 100)} className="h-2" />
              <div className="text-xs text-muted-foreground">
                理想: &lt;50ms
              </div>
            </div>
          ...
          <>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center justify-between text-sm">
                  ...
                  {getTrendIcon(currentData.renderTime, previousData.renderTime)}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className={`text-2xl font-bold ${getStatusColor(
                    getPerformanceStatus(currentData.renderTime, defaultThresholds.renderTime)
                  )}`}>
                    {currentData.renderTime.toFixed(1)}ms
                  </div>
                  <Progress value={Math.min((currentData.renderTime / 33) * 100, 100)} className="h-2" />
                  <div className="text-xs text-muted-foreground">
                    目标: &lt;16ms
                  </div>
                </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className={`text-2xl font-bold ${getStatusColor(
                    getPerformanceStatus(currentData.domNodes, defaultThresholds.domNodes)
                  )}`}>
                    {currentData.domNodes}
                  </div>
                  <Progress value={Math.min((currentData.domNodes / 2000) * 100, 100)} className="h-2" />
                  <div className="text-xs text-muted-foreground">
                    建议: &lt;1000
                  </div>
                </div>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center justify-between text-sm">
                  ...
                  {getTrendIcon(currentData.eventListeners, previousData.eventListeners)}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
