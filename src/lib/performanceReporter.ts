...
interface ComponentRenderMetric { componentName: string;
  renderTime: number;
  renderCount: number;
  propsChanges: number;,
}

class PerformanceReporter { private metrics: PerformanceMetric[] = [];
  ...
  // 记录组件渲染性能
  recordComponentRender(componentName: string, renderTime: number, propsChanged: boolean = false) { const existing = this.renderMetrics.get(componentName) || {
      componentName,
      renderTime: 0,
      renderCount: 0,
      propsChanges: 0,
    ...
    const updated: ComponentRenderMetric = { ...existing,
    ...
    // 检查渲染性能
    if (updated.renderCount % 10 === 0 && updated.renderTime > 16) { logger.warn(`组件渲染性能警告: ${componentName }`, { averageRenderTime: updated.renderTime,
  ...
  // 记录Web Vitals
  recordWebVitals() { if (this.vitalsReported || typeof window === 'undefined') return;
    ...
    if ('PerformanceObserver' in window) {
      ...
      try { fidObserver.observe({ type: 'first-input', buffered: true  });,
} catch (e) { // 浏览器可能不支持,
}
  ...
  // 检查性能阈值
  private checkPerformanceThresholds(metric: PerformanceMetric) { const thresholds: Record<string, number> = {
      'LCP': 2500, // ms
      'FID': 100,  // ms
      'CLS': 0.1,  // score
      'TTFB': 800, // ms
    ...
    if (threshold && metric.value > threshold) { logger.warn(`性能阈值超标: ${metric.name }`, { value: metric.value,
        threshold,
        unit: metric.unit,
  ...
  // 获取性能报告
  getPerformanceReport() { const now = Date.now();
    ...
    // 计算统计信息
    const statistics = Object.entries(metricsByType).map(([name, values]) => ({ name,
      count: values.length,
  ...
  // 计算百分位数
  private percentile(values: number[], p: number): number { const sorted = [...values].sort((a, b) => a - b);
    const index = Math.ceil((p / 100) * sorted.length) - 1;
    return sorted[index] || 0;,
}

  // 清理旧数据
  cleanup() { const cutoff = Date.now() - 24 * 60 * 60 * 1000; // 24小时前
...
// 自动记录Web Vitals
if (typeof window !== 'undefined') { performanceReporter.recordWebVitals();
