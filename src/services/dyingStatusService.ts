...
export interface DyingStatusResolution {
  userId: string;
  gameStateId: string;
...
export class DyingStatusService {
   ...
   * @returns 濒死状态玩家列表
   */
  static async getDyingPlayers(roomId: string) {
    try {
      ...
      if (error) {
        console.error('获取濒死玩家失败:', error);
        return [];
      }

      return data || [];
    } catch (error) {
      console.error('获取濒死玩家异常:', error);
      return [];
    }
  }
  ...
  static async isPlayerDying(
    ...
    try {
      ...
      if (error) {
        console.error('检查濒死状态失败:', error);
        return false;
      }

      return data?.role_status === 2;
    } catch (error) {
      console.error('检查濒死状态异常:', error);
      return false;
    }
  }
