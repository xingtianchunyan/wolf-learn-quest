...
public async querySkillData(options: SkillDataQueryOptions): Promise< { skillUses: EnhancedSkillUse[];
    ...
    total: number
      ...
      *,
      ...
      // 查询技能效果（如果需要）
      if (options.includeEffects && options.gameStateId) { const { data: skillEffects  
} = await supabase;
        .from('skill_effects_queue')
        .select('*')
      ...
      // 查询技能目标（如果需要）
      if (options.includeTargets && options.gameStateId) { const { data: skillTargets  
} = await supabase;
        .from('skill_targets')
        .select('*')
        .eq('game_state_id', options.gameStateId)
...
public async getSkillStats(gameStateId: string): Promise<SkillSystemStats> { try  {
      ...
      // 查询技能使用数据
      const { data: skillUses, error: skillUsesError  
} = await supabase;
      .from('skill_uses')
      ...
      // 查询活跃效果数量
      const { count: activeEffectsCount, error: effectsError  
} = await supabase;
      .from('skill_effects_queue')
  ...
  * @param olderThanHours - 清理多少小时前的数据
  ...
  public async cleanupExpiredSkillData(
    ...
    deletedEffects: number
      ...
      // 清理过期的技能使用记录
      const { count: deletedSkillUses, error: skillUsesError  
} = await supabase;
      .from('skill_uses')
      .delete({ count: 'exact'  
})
      ...
      // 清理过期的技能效果
      const { count: deletedEffects, error: effectsError  
} = await supabase;
      .from('skill_effects_queue')
      .delete({ count: 'exact'  
})
