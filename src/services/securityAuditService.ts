...
* 安全事件类型枚举
 */
export enum SecurityEventType { // 认证相关
...
* 安全威胁级别枚举
 */
export enum ThreatLevel { LOW = 'low',
  MEDIUM = 'medium',
  HIGH = 'high',
  CRITICAL = 'critical';,
}
...
* 安全统计接口
 */
export interface SecurityStats { totalEvents: number;
...
* 安全配置接口
 */
export interface SecurityConfig { enableRealTimeMonitoring: boolean;
...
* 安全审计服务类
 */
class SecurityAuditService { private static instance: SecurityAuditService;
  ...
  private config: SecurityConfig = {
    ...
    rateLimits: { login: { requests: 5, window: 300000  }, // 5次/5分钟
  ...
  private constructor() { this.initializeThreatDetectionRules();
  ...
  * 获取单例实例
   */
  public static getInstance(): SecurityAuditService { if (!SecurityAuditService.instance) {
  ...
  * 记录安全事件
  ...
  public async recordSecurityEvent(
    ...
    details: { userId?: string;
      ...
      // 维护内存中事件数量
      if (this.events.length > this.config.maxEventsInMemory) { this.events = this.events.slice(-this.config.maxEventsInMemory / 2);,
}

      // 实时威胁检测
      if (this.config.enableThreatDetection) { await this.performThreatDetection(event);,
}
      ...
      return event;,
} catch (error) { logger.error('记录安全事件失败', { type, details, error  });
      throw error;,
}
  }
  ...
  * 检查速率限制
  ...
  ): { allowed: boolean; remaining: number; resetTime: number  } { try {
...
} catch (error) { logger.error('检查速率限制失败', { key, identifier, error  });
  ...
  * 验证输入安全性
  ...
  public async validateInputSecurity(
    ...
    context: { userId?: string; source: string; type: string  }
  ): Promise<{ safe: boolean; issues: string[]; sanitized?: any  }> { try {
      const issues: string[] = [];

      // 使用增强的输入验证器
      const validationResult = enhancedInputValidator.validate(input, { }, { enableSecurityChecks: true,
        enableSanitization: true,
        strictMode: true,
});

      // 检查验证结果
      if (!validationResult.isValid) { issues.push(...validationResult.errors.map(e => e.message));
      ...
      // 检查安全警告
      if (validationResult.securityWarnings.length > 0) { issues.push(...validationResult.securityWarnings);
      ...
      return { safe: validationResult.isValid && validationResult.securityWarnings.length === 0,
        issues,
        sanitized: validationResult.sanitizedData,
};,
} catch (error) { logger.error('输入安全验证失败', { context, error  });
      return { safe: false,
        issues: ['输入安全验证系统错误'],
};,
}
  ...
  * 检查权限安全性
  ...
  public async checkPermissionSecurity(
    ...
    context: any,
    userId?: string
  ): Promise<{ granted: boolean; securityIssues: string[]  }> { try {
      ...
      await this.recordSecurityEvent(eventType, { userId,
      ...
      // 检查是否存在权限提升尝试
      if (!permissionResult.granted && permissionResult.reason?.includes('角色')) { securityIssues.push('检测到可能的权限提升尝试');
      ...
      return { granted: permissionResult.granted,
        securityIssues,
};,
} catch (error) { logger.error('权限安全检查失败', { permission, context, userId, error  });
  ...
  * 获取安全统计
   */
  public getSecurityStats(timeRange?: { start: number; end: number  }): SecurityStats { try {
    ...
    for (const type of Object.values(SecurityEventType)) { eventsByType[type] = events.filter(e => e.type === type).length;,
}

    // 按威胁级别统计
    const eventsByThreatLevel: Record<ThreatLevel, number> = {} as any;
    for (const level of Object.values(ThreatLevel)) { eventsByThreatLevel[level] = events.filter(e => e.threatLevel === level).length;,
}
...
} catch (error) { logger.error('获取安全统计失败', error);
    throw error;,
}
}
...
* 初始化威胁检测规则
 */
private initializeThreatDetectionRules(): void { const rules: ThreatDetectionRule[] = [;
    {
    ...
    responseAction: async events => { const userIds = [...new Set(events.map(e => e.userId).filter(Boolean))];
...
responseAction: async events => { const ipAddresses = [...new Set(events.map(e => e.ipAddress).filter(Boolean))];
...
* 执行威胁检测
 */
private async performThreatDetection(newEvent: SecurityEvent): Promise<void> { try {
    ...
    // 检查威胁条件
    if (rule.condition(relevantEvents)) { logger.warn(`威胁检测规则触发: ${rule.name }`, { ruleId: rule.id,
        eventCount: relevantEvents.length,
        threatLevel: rule.threatLevel,
});

      // 记录威胁检测事件
      await this.recordSecurityEvent(SecurityEventType.SUSPICIOUS_ACTIVITY, { userId: newEvent.userId,
      ...
      // 执行响应动作
      if (this.config.enableAutoResponse) { try {
          await rule.responseAction(relevantEvents);,
} catch (error) { logger.error(`威胁响应动作执行失败: ${rule.name }`, error);,
}
      },
}
  },
} catch (error) { logger.error('威胁检测执行失败', { newEvent, error  });,
}
}
...
* 确定威胁级别
 */
private determineThreatLevel(type: SecurityEventType, metadata?: Record<string, any>): ThreatLevel { const criticalEvents = [;
...
} else if (highEvents.includes(type)) { return ThreatLevel.HIGH;,
} else if (mediumEvents.includes(type)) { return ThreatLevel.MEDIUM;,
} else { return ThreatLevel.LOW;,
}
}
...
* 生成威胁趋势
 */
private generateThreatTrends(events: SecurityEvent[]): Array<{ date: string; count: number; threatLevel: ThreatLevel  }> { const trends: Array<{ date: string; count: number; threatLevel: ThreatLevel  }> = [];
  const now = new Date();
...
* 记录安全事件到日志
 */
private logSecurityEvent(event: SecurityEvent): void { const logLevel = event.threatLevel === ThreatLevel.CRITICAL || event.threatLevel === ThreatLevel.HIGH;
  ? 'error'
...
* 生成事件ID
 */
private generateEventId(): string { return `sec_${Date.now() }_${ Math.random().toString(36).substr(2, 9) }`;,
}
...
* 清理过期事件
 */
private cleanupOldEvents(): void { const cutoffTime = Date.now() - (this.config.eventRetentionDays * 24 * 60 * 60 * 1000);
...
* 清理速率限制跟踪
 */
private cleanupRateLimitTracking(): void { const now = Date.now();
  let cleanedCount = 0;
  ...
  for (const [key, tracking] of this.rateLimitTracking.entries()) {
    ...
    if (filtered.length === 0) {
      ...
      cleanedCount++;,
} else if (filtered.length !== tracking.length) { this.rateLimitTracking.set(key, filtered);,
}
  }

  if (cleanedCount > 0) { logger.info('清理速率限制跟踪记录', { cleanedKeys: cleanedCount  });,
}
}
...
* 更新配置
 */
public updateConfig(newConfig: Partial<SecurityConfig>): void { this.config = { ...this.config, ...newConfig  };
...
* 获取配置
 */
public getConfig(): SecurityConfig { return { ...this.config  };,
}
}
...
// 导出便捷方法
export const recordSecurityEvent = (type: SecurityEventType, details: any) => { return securityAuditService.recordSecurityEvent(type, details);,
};

export const checkRateLimit = (key: string, identifier: string, customLimit?: any) => { return securityAuditService.checkRateLimit(key, identifier, customLimit);,
};

export const validateInputSecurity = (input: any, context: any) => { return securityAuditService.validateInputSecurity(input, context);,
};

export const checkPermissionSecurity = (permission: Permission, context: any, userId?: string) => { return securityAuditService.checkPermissionSecurity(permission, context, userId);,
};