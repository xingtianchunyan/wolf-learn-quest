...
class SecurityAuditService  { private static instance: SecurityAuditService;
  ...
  private config: SecurityConfig = {
    ...
    rateLimits: { login: { requests: 5, window: 300000  
}, // 5次/5分钟
      api: { requests: 100, window: 60000  
},   // 100次/分钟
      skill_use: { requests: 10, window: 60000  
} // 10次/分钟 }
  };
  private rateLimitTracking: Map<string, Array<{ timestamp: number; count: number  
}>> = new Map();
 ...
 * 获取安全统计
 */
public getSecurityStats(timeRange?: { start: number; end: number  
}): SecurityStats { try  {
...
private async performThreatDetection(newEvent: SecurityEvent): Promise<void> { try  {
    ...
    // 检查威胁条件
    if (rule.condition(relevantEvents)) { logger.warn(`威胁检测规则触发: ${rule.name 
}`, { ruleId: rule.id,
...
} catch (error) { logger.error(`威胁响应动作执行失败: ${rule.name 
}`, error)
}
      } }
 ...
 * 生成事件ID
 */
private generateEventId(): string { return `sec_${Date.now() 
