...
export interface SkillEffect { skillUseId: string;
  userId: string;
  targetUserId: string;
  effectType: string;
  skillName: string;
...
export class PassiveSkillService { /**
  * 处理恶魔免疫狼杀的被动技能 - 增强版本，使用数据库函数
  ...
  static async processDemonImmunity(
    ...
    gameStateId: string,
    roomId: string
  ): Promise<{ isImmune: boolean; reason?: string  }> { try {
      logger.debug('检查恶魔免疫', { targetUserId, attackerUserId, gameStateId  });

      // 使用数据库函数检查恶魔免疫
      const { data, error  } = await supabase.rpc('check_demon_immunity', { p_target_user_id: targetUserId,
      ...
      if (isImmune) { logger.info('恶魔免疫狼人攻击', { targetUserId, attackerUserId  });

        // 创建系统公告
        await SystemAnnouncementService.createSkillUsageAnnouncement({ type: 'skill_usage',
      ...
      return { isImmune: false  };,
} catch (error) { logger.error('恶魔免疫处理异常', error);
      return { isImmune: false  };,
}
  }
  ...
  * 处理多重保护导致淘汰的逻辑 - 增强版本，使用数据库函数
  ...
  static async processMultipleProtection(
    ...
    roomId: string,
    currentRound: number
  ): Promise<{ shouldEliminate: boolean; reason?: string  }> { try {
      ...
      if (shouldEliminate) { logger.info('多重保护导致淘汰', {
          targetUserId,
        ...
        // 创建系统公告
        await SystemAnnouncementService.createStatusChangeAnnouncement({ type: 'status_change',
          actorUserId: '',
          actorName: '',
          actorRole: '',
          targetUserId,
        ...
        return { shouldEliminate: true,
          reason: (data as any)?.reason || '多重保护导致淘汰',
};,
}

      return { shouldEliminate: false  };,
} catch (error) { logger.error('多重保护处理异常', error);
      return { shouldEliminate: false  };,
}
  }
  ...
  * 处理猎人濒死被动技能触发 - 增强版本，使用数据库函数
  ...
  static async processHunterDyingTrigger(
    ...
    triggerReason: string = 'elimination';
      ...
      if (success) { logger.info('猎人濒死技能触发成功', { hunterUserId, triggerReason  });

        // 发送猎人濒死广播
        await SystemAnnouncementService.createHunterDeathBroadcast({ type: 'hunter_broadcast',
      ...
      return success;,
} catch (error) { logger.error('猎人濒死技能处理异常', error);
      return false;,
}
  }
  ...
  * 处理技能冲突解决
  ...
  static async resolveSkillConflicts(
    ...
    gameStateId: string
      ...
      for (const skill of sortedSkills) { const hasConflictWithResolved = resolvedSkills.some(resolved =>;
        this.skillsConflict(skill, resolved)
      );

      if (!hasConflictWithResolved) {
        resolvedSkills.push(skill);,
} else { cancelledSkills.push(skill);,
}
    }

    // 取消冲突的技能
    for (const skill of cancelledSkills) { await supabase
      .from('skill_uses')
      .update({
...
} catch (error) { console.error('Error resolving skill conflicts:', error);
...
* 检查技能列表是否存在冲突
 */
private static checkSkillConflicts(skills: SkillEffect[]): boolean { for (let i = 0; i < skills.length; i++) {
...
* 检查两个技能是否冲突
 */
private static skillsConflict(skill1: SkillEffect, skill2: SkillEffect): boolean { // 守卫保护与狼人攻击冲突
  ...
  skill2.skillName === 'magic_potion' && skill2.effectType === 'protection')) { return skill1.targetUserId === skill2.targetUserId;,
}
