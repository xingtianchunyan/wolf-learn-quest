...
export class RoomService {
  private static sanitizeError(error: any): string {
    ...
    if (error.code === '23505') { // Unique constraint violation
      ...
      if (error.message?.includes('role_selections_unique_room_user')) {
        return '你已经选择了角色，请先取消当前选择';
      }
      return '选择冲突，请重试';
    }
  ...
  private static async validateUserId(providedUserId: string): Promise<void> {
