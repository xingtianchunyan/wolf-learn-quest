...
class MonitoringService { private metrics: PerformanceMetric[] = [];
 ...
 * 记录性能指标
 */
recordMetric(metric: PerformanceMetric): void { this.metrics.push( {
      ...metric,
      timestamp: Date.now() 
});

    // 限制存储的指标数量
    if (this.metrics.length > this.MAX_METRICS) { this.metrics = this.metrics.slice(-this.MAX_METRICS)
}
 ...
 * 检查指标阈值并触发告警
 */
private checkThresholds(metric: PerformanceMetric): void  { let alert: Alert | null = null;
    switch (metric.name) {
      case 'response_time':
      ...
      case 'memory_usage':
      if (metric.value > this.ALERT_THRESHOLDS.memoryUsage) { alert = {
          id: `alert-${Date.now() 
}`,
          severity: 'warning',
          message: `内存使用过高: ${ (metric.value / 1024 / 1024).toFixed(2) 
}MB`,
          timestamp: Date.now(),
          resolved: false 
}
      ...
      case 'error_rate':
      if (metric.value > this.ALERT_THRESHOLDS.errorRate) { alert = {
          id: `alert-${Date.now() 
}`,
          severity: 'error',
 ...
 * 获取系统健康状态
 */
async getSystemHealth(): Promise<SystemHealth>  { const now = Date.now();
    ...
    if (activeAlerts.some(a => a.severity === 'critical')) {
      status = 'critical'
} else if (activeAlerts.some(a => a.severity === 'error' || a.severity === 'warning')) { status = 'warning'
}
 ...
 * 获取活跃用户数
 */
private async getActiveUserCount(): Promise<number> { try  {
      const { data, error  } = await supabase;
      .from('rooms')
      .select('id')
      .eq('status', 'active');

      if (error) { logger.error('获取活跃用户数失败', error);
        return 0
}

      return data?.length || 0
} catch (error) { logger.error('获取活跃用户数异常', error);
      return 0
}
  }
 ...
 * 获取指标历史
 */
getMetrics(options?:  { name?: string;
    startTime?: number;
    endTime?: number;
    limit?: number
}): PerformanceMetric[] { let filtered = this.metrics;

    if (options?.name) {
      filtered = filtered.filter(m => m.name === options.name)
}

    if (options?.startTime) { filtered = filtered.filter(m => m.timestamp >= options.startTime!)
}

    if (options?.endTime) { filtered = filtered.filter(m => m.timestamp <= options.endTime!)
}
 ...
 * 清理旧数据
 */
cleanup(maxAge: number = 3600000): void  { const now = Date.now();
 ...
 * 获取性能报告
 */
getPerformanceReport(timeRange: number = 3600000): { summary:  {
      totalMetrics: number;
    ...
    return {
      ...
      trends: { responseTime: responseTimeMetrics.map(m => m.value),
