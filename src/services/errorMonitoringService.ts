...
* 错误类型枚举
 */
export enum ErrorType  { /** JavaScript错误 */
...
* 性能指标类型枚举
 */
export enum PerformanceMetricType  { /** 页面加载时间 */
  PAGE_LOAD = 'page_load',
  /** 首次内容绘制 */
...
* 错误报告接口
 */
export interface ErrorReport  { /** 错误ID */
  id: string;
  /** 错误级别 */
  level: ErrorLevel;
  /** 错误类型 */
...
* 浏览器信息接口
 */
export interface BrowserInfo  { /** 浏览器名称 */
  name: string;
  /** 浏览器版本 */
  version: string;
  /** 渲染引擎 */
...
* 设备信息接口
 */
export interface DeviceInfo  { /** 设备类型 */
...
* 用户操作接口
 */
export interface UserAction  { /** 操作类型 */
...
* 监控配置接口
 */
export interface MonitoringConfig  { /** 是否启用错误监控 */
...
* 监控统计接口
 */
export interface MonitoringStats  { /** 总错误数 */
  totalErrors: number;
  /** 按级别统计 */
...
* 提供全面的错误监控和分析功能
 */
export class ErrorMonitoringService  { private static instance: ErrorMonitoringService;
  ...
  private constructor() {
    ...
    this.stats = { totalErrors: 0,
      ...
      errorsByType: { [ErrorType.JAVASCRIPT]: 0,
        [ErrorType.NETWORK]: 0,
        [ErrorType.API]: 0,
        [ErrorType.UI]: 0,
 ...
 * 获取单例实例
 */
public static getInstance(): ErrorMonitoringService { if (!ErrorMonitoringService.instance)  {
...
public async initialize(): Promise<void> { if (this.initialized)  {
    ...
    try { // 设置全局错误处理器
      ...
      // 设置用户行为追踪
      if (this.config.enableUserTracking) { this.setupUserTracking()
}

      // 设置自动上报
      if (this.config.enableAutoReporting) { this.setupAutoReporting()
}

      this.initialized = true;
      logger.info('错误监控服务初始化完成')
} catch (error) { logger.error('错误监控服务初始化失败', error);
      throw error
}
  }
  ...
  * @param config - 监控配置
   */
public configure(config: Partial<MonitoringConfig>): void { this.config =  { ...this.config, ...config   };
  ...
  * @param type - 错误类型
  ...
  public async reportError(
    ...
    type: ErrorType = ErrorType.JAVASCRIPT;
      ...
      // 立即上报严重错误
      if (level === ErrorLevel.CRITICAL) { await this.sendErrorReport(errorReport)
}

      logger.info(`错误已报告: ${ errorReport.id 
}`, errorReport);
      return errorReport.id
} catch (reportingError) { logger.error('报告错误失败', reportingError);
  ...
  * @param context - 上下文信息
  ...
  public async reportPerformanceMetric(
    ...
    context?: any
      ...
      logger.debug(`性能指标已报告: ${ type 
} = ${ value }`)
} catch (error) { logger.error('报告性能指标失败', error)
}
  }
  ...
  * @param action - 用户操作
   */
public reportUserAction(action: UserAction): void { try  {
      ...
      // 保持历史大小限制
      if (this.userActions.length > this.config.maxUserActions) { this.userActions = this.userActions.slice(0, this.config.maxUserActions)
}
...
} catch (error) { logger.error('报告用户操作失败', error)
}
  }
  ...
  * @param event - 安全事件
   */
public async reportSecurityEvent(event: SecurityEvent): Promise<void> { try  {
      // 将安全事件转换为错误报告
...
} catch (error) { logger.error('报告安全事件失败', error)
}
  }
 ...
 * 获取错误统计
 */
public getStats(): MonitoringStats { return  { ...this.stats  
}
}
  /**
 * 获取错误缓存
 */
public getErrorCache(): ErrorReport[]  { return [...this.errorCache]
}
  /**
 * 获取用户操作历史
 */
public getUserActions(): UserAction[]  { return [...this.userActions]
}
  /**
 * 清空缓存
 */
public clearCache(): void  { this.errorCache = [];
    this.userActions = [];
    this.stats = {
      totalErrors: 0,
      errorsByLevel: {
 ...
 * 手动上报所有缓存的错误
 */
public async flushErrors(): Promise<void> { try  {
      ...
      for (const error of errors) { await this.sendErrorReport(error)
}

      logger.info(`已上报 ${ errors.length } 个缓存错误`)
} catch (error) { logger.error('上报缓存错误失败', error)
}
  }
  ...
  * 设置全局错误处理器
   */
private setupGlobalErrorHandlers(): void  { // JavaScript错误
  ...
  * 设置性能监控
   */
private setupPerformanceMonitoring(): void  { // Web Vitals监控
    if ('PerformanceObserver' in window) {
      ...
      // 监控各种性能指标
      try { this.performanceObserver.observe({ entryTypes: ['navigation', 'paint', 'largest-contentful-paint', 'first-input', 'layout-shift']  })
} catch (error) { logger.warn('部分性能指标监控不支持', error)
}
    }
  ...
  * 设置用户行为追踪
   */
private setupUserTracking(): void  { // 点击事件
    ...
    // 导航事件
    window.addEventListener('popstate', () => { this.reportUserAction({
        type: 'navigation',
 ...
 * 设置自动上报
 */
private setupAutoReporting(): void { this.reportingTimer = setInterval(async () =>  {
      try {
        await this.flushErrors()
} catch (error) { logger.error('自动上报失败', error)
}
...
private createErrorReport(
    ...
    level: ErrorLevel = ErrorLevel.ERROR,
    type: ErrorType = ErrorType.JAVASCRIPT;
  ): ErrorReport { const errorMessage = error instanceof Error ? error.message : String(error);
 ...
 * 创建安全错误报告
 */
private createSecurityErrorReport(event: SecurityEvent): ErrorReport { return  {
      id: this.generateErrorId(),
      ...
      browser: this.getBrowserInfo(),
      device: this.getDeviceInfo(),
      context: { component: 'SecuritySystem',
      ...
      userActions: [...this.userActions],
      performance: this.getCurrentPerformanceMetrics(),
      customData: { securityEvent: event 
},
      handled: false 
}
}
  ...
  * 检查是否应该忽略错误
   */
private shouldIgnoreError(errorReport: ErrorReport): boolean  { // 检查忽略的错误消息
    if (this.config.ignoredErrors.some(ignored => errorReport.message.includes(ignored))) {
      return true
}

    // 检查忽略的URL模式
    if (this.config.ignoredUrls.some(pattern => pattern.test(errorReport.url))) { return true
}

    // 检查自定义过滤器
    if (this.config.customFilters.some(filter => !filter(errorReport))) { return true
}
 ...
 * 添加到缓存
 */
private addToCache(errorReport: ErrorReport): void  { this.errorCache.unshift(errorReport);
    // 保持缓存大小限制
 ...
 * 更新统计信息
 */
private updateStats(errorReport: ErrorReport): void  { this.stats.totalErrors++;
 ...
 * 发送错误报告
 */
private async sendErrorReport(errorReport: ErrorReport): Promise<void> { if (!this.config.reportingEndpoint)  {
      logger.debug('未配置上报端点，跳过错误上报');
      return
}

    try { const response = await fetch(this.config.reportingEndpoint, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) { throw new Error(`上报失败: ${response.status 
} ${ response.statusText }`)
}
...
} catch (error) { logger.error('发送错误报告失败', error);
      throw error
}
  }
...
private processPerformanceEntry(entry: PerformanceEntry): void { switch (entry.entryType)  {
      ...
      if (!lsEntry.hadRecentInput) { this.reportPerformanceMetric(
 ...
 * 收集页面加载指标
 */
private collectPageLoadMetrics(): void { if ('performance' in window && 'getEntriesByType' in performance)  {
 ...
 * 更新性能指标
 */
private updatePerformanceMetrics(type: PerformanceMetricType, value: number): void { switch (type)  {
...
private async checkPerformanceThresholds(
    ...
    if (threshold && value > threshold) { await this.reportError(
        `Performance threshold exceeded: ${type 
} = ${ value }ms (threshold: ${ threshold 
}ms)`,
 ...
 * 获取当前性能指标
 */
private getCurrentPerformanceMetrics(): PerformanceMetrics { return  { ...this.stats.performanceMetrics  
}
}
  /**
 * 获取浏览器信息
 */
private getBrowserInfo(): BrowserInfo  { const userAgent = navigator.userAgent;
    // 简单的浏览器检测
    let name = 'Unknown';
    let version = 'Unknown';
    let engine = 'Unknown';
 ...
 * 获取设备信息
 */
private getDeviceInfo(): DeviceInfo  { const screen = window.screen;
 ...
 * 获取元素选择器
 */
private getElementSelector(element: Element): string  { if (!element) return 'unknown';
    if (element.id) {
      return `#${element.id }`
}
 ...
 * 映射严重级别到错误级别
 */
private mapSeverityToLevel(severity: string): ErrorLevel { switch (severity)  {
      case 'critical':
