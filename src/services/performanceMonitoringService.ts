...
export class PerformanceMonitoringService { private static instance: PerformanceMonitoringService;
  ...
  * 启动定期报告
  ...
  private startPeriodicReporting(): void { if (this.reportingTimer) {
    ...
    this.reportingTimer = setInterval(() => { this.generatePerformanceReport();,
  ...
  * @param metric - 性能指标
  ...
  private checkThresholds(metric: PerformanceMetric): void { const threshold = this.config.thresholds.find(t => t.type === metric.type);
    ...
    if (severity) { const issue: PerformanceIssue = {
        ...
        description: `${threshold.description }超过${ severity === 'critical' ? '严重' : '警告' }阈值`,
        detectedAt: new Date(),
};
  ...
  * @returns 影响描述
  ...
  private calculateImpact(metric: PerformanceMetric, threshold: PerformanceThreshold): string { const exceedPercentage = ((metric.value - threshold.warningThreshold) / threshold.warningThreshold * 100).toFixed(1);
    ...
    switch (metric.type) {
      ...
      return `渲染性能下降${exceedPercentage }%，可能导致界面卡顿`;
      case PerformanceMetricType.MEMORY_USAGE:
      return `内存使用超标${ exceedPercentage }%，可能导致应用崩溃`;
      case PerformanceMetricType.API_RESPONSE_TIME:
      return `API响应延迟${ exceedPercentage }%，影响用户体验`;
      case PerformanceMetricType.COMPONENT_MOUNT_TIME:
      return `组件加载缓慢${ exceedPercentage }%，影响页面响应速度`;
      case PerformanceMetricType.HOOK_EXECUTION_TIME:
      return `Hook执行耗时${ exceedPercentage }%，可能阻塞UI更新`;
      case PerformanceMetricType.CACHE_HIT_RATE:
      return `缓存命中率低${ exceedPercentage }%，增加不必要的计算`;
      case PerformanceMetricType.ERROR_RATE:
      return `错误率高${ exceedPercentage }%，影响应用稳定性`;
      case PerformanceMetricType.USER_INTERACTION_DELAY:
      return `交互延迟${ exceedPercentage }%，影响用户操作体验`;
      default:
      return `性能指标异常，超出预期${ exceedPercentage }%`;,
}
  }
  ...
  * @param data - 事件数据
  ...
  private emitPerformanceEvent(eventType: string, data: any): void { try {
      const event = new CustomEvent(`performance-monitoring:${eventType }`, { detail: data,
});
      ...
      for (const [key, metrics] of this.metrics.entries()) { if (metrics.length === 0) continue;
        ...
        if (threshold) { if (latestMetric.value >= threshold.criticalThreshold) {
            analysis.criticalIssues.push({
              ...
              description: `${threshold.description }严重超标`,
              detectedAt: new Date(),
});,
              ...
              thresholdValue: threshold.warningThreshold,
              impact: this.calculateImpact(latestMetric, threshold),
              description: `${threshold.description }超出警告阈值`,
              detectedAt: new Date(),
});,
}
        }
  ...
  * @returns 分数 (0-100)
  ...
  private calculateMetricScore(metric: PerformanceMetric): number { const threshold = this.config.thresholds.find(t => t.type === metric.type);
    ...
    if (metric.value <= threshold.warningThreshold) { return 100; // 优秀,
} else if (metric.value <= threshold.criticalThreshold) { // 在警告和严重阈值之间，线性计算分数
      const ratio = (metric.value - threshold.warningThreshold) /;
      (threshold.criticalThreshold - threshold.warningThreshold);
      return Math.max(0, Math.round(100 - (ratio * 50))); // 100分降到50分,
} else { // 超过严重阈值，根据超出程度计算分数
  ...
  * @returns 趋势信息
  ...
  private calculateTrend(metrics: PerformanceMetric[]): PerformanceTrend | null { if (metrics.length < 2) {
    ...
    return { type: metrics[0].type,
      ...
      timeRange: `${recentMetrics.length }个数据点`,
  ...
  private generateRecommendationForIssue(
    ...
    priority: 'high' | 'medium' | 'low'
      ...
      return {
        ...
        priority,
        title: '优化组件渲染性能',
        description: `${issue.componentName || '组件' }渲染时间过长，需要优化渲染逻辑`,
      ...
      case PerformanceMetricType.MEMORY_USAGE:
      return { id: this.generateMetricId(),
        category: 'memory',
        priority,
        title: '优化内存使用',
  ...
  * @returns 优化建议
  ...
  private generateRecommendationForTrend(trend: PerformanceTrend): PerformanceRecommendation | null { return {
      ...
      category: 'rendering',
      priority: 'medium',
      title: `关注${trend.type }性能下降趋势`,
      description: `${ trend.type }指标呈现下降趋势，变化幅度${ trend.changePercentage.toFixed(1) }%`,
  ...
  * 生成指标ID
   */
  private generateMetricId(): string { return `metric_${Date.now() }_${ Math.random().toString(36).substr(2, 9) }`;,
}
