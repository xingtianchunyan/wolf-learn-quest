...
export enum PermissionStrategy {
  ALL_REQUIRED = 'all_required',     // 需要所有权限
...
export class PermissionMiddleware {
  ...
  // 缓存和监控
  private checkCache: Map<string, { result: PermissionCheckResult; timestamp: number }> = new Map();
  ...
  // 配置
  private globalConfig: Partial<PermissionMiddlewareConfig> = {
    mode: PermissionCheckMode.STRICT,
  ...
  public requirePermission(
    ...
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (!userId) {
          return this.handleUnauthorized(req, res, '用户未认证');
        }
        ...
        if (normalizedConfig.cache?.enabled) {
          ...
          if (cached && Date.now() - cached.timestamp < (normalizedConfig.cache.ttl || 300000)) {
            if (cached.result.allowed) {
              ...
              return this.handlePermissionDenied(req, res, cached.result, normalizedConfig);
            }
          }
        }
        ...
        // 缓存结果
        if (normalizedConfig.cache?.enabled) {
          this.cacheResult(cacheKey, checkResult);
        }

        // 记录审计日志
        if (normalizedConfig.audit?.enabled) {
          this.recordAudit(req, checkResult, normalizedConfig, performance.now() - startTime);
        }

        // 处理检查结果
        if (checkResult.allowed) {
          this.attachPermissionInfo(req, checkResult);
          next();
        } else {
          this.handlePermissionDenied(req, res, checkResult, normalizedConfig);
        }

      } catch (error) {
        this.handleMiddlewareError(error, req, res, next, config);
      } finally {
        // 记录性能指标
   ...
   * 基于角色的权限检查
   */
  public requireRole(roles: string | string[], options?: Partial<PermissionMiddlewareConfig>) {
   ...
   * 使用自定义检查函数
   */
  public requireCustomPermission(
    customCheck: (req: AuthenticatedRequest, context: PermissionContext) => Promise<boolean>,
    options?: Partial<PermissionMiddlewareConfig>
  ) {
    const config: PermissionMiddlewareConfig = {
      ...this.globalConfig,
   ...
   * 同时检查多个权限
   */
  public requireAnyPermission(permissions: string[], options?: Partial<PermissionMiddlewareConfig>) {
    const config: PermissionMiddlewareConfig = {
      ...this.globalConfig,
      ...options,
   ...
   * 函数级注释：更新全局配置
   */
  public updateGlobalConfig(config: Partial<PermissionMiddlewareConfig>): void {
   ...
   * 将不同格式的配置标准化
   */
  private normalizeConfig(config: string | string[] | PermissionMiddlewareConfig): PermissionMiddlewareConfig {
  ...
  private extractUserId(req: AuthenticatedRequest): string | null {
    ...
    if (req.headers.authorization) {
      try {
        ...
        logger.warn('JWT token解析失败', { error });
      }
    }
   ...
   * 构建权限检查所需的上下文信息
   */
  private buildPermissionContext(req: AuthenticatedRequest, config: PermissionMiddlewareConfig): PermissionContext {
    return {
   ...
   * 为权限检查结果生成缓存键
   */
  private generateCacheKey(req: AuthenticatedRequest, config: PermissionMiddlewareConfig): string {
    ...
    return `${userId}:${endpoint}:${method}:${permissions}:${roles}`;
  }
  ...
  private async performPermissionCheck(
    ...
    try {
      ...
      switch (config.strategy) {
        case PermissionStrategy.ALL_REQUIRED:
        ...
        case PermissionStrategy.CUSTOM:
          if (config.customCheck) {
            allowed = await config.customCheck({ user: { id: userId } } as AuthenticatedRequest, context);
          }
          break;
      ...
      // 检查条件
      if (allowed && config.conditions && config.conditions.length > 0) {
        allowed = this.evaluateConditions(config.conditions, context);
      }
      ...
      logger.error('权限检查失败', { error, userId, config });
      return {
        allowed: false,
        reason: '权限检查过程中发生错误',
        permissions: [],
  ...
  private async checkAllPermissions(
    ...
    for (const permissionName of permissionNames) {
      try {
        ...
        results.push({ name: permissionName, granted: false, reason: '检查失败' });
        allGranted = false;
      }
    }
   ...
   * 将权限信息附加到请求对象
   */
  private attachPermissionInfo(req: AuthenticatedRequest, result: PermissionCheckResult): void {
    req.permissions = {
   ...
   * 处理用户未认证的情况
   */
  private handleUnauthorized(req: Request, res: Response, reason: string): void {
    res.status(401).json({
      error: 'Unauthorized',
      message: reason,
      code: 'UNAUTHORIZED',
  ...
  private handleMiddlewareError(
    ...
    logger.error('权限中间件错误', { error, path: req.path, method: req.method });

    const normalizedConfig = this.normalizeConfig(config);
    
    if (normalizedConfig.onError) {
      normalizedConfig.onError(error, req, res);
      return;
    }
  ...
  private recordAudit(
    ...
    if (config.audit?.includeRequestData) {
      auditRecord.requestData = {
        query: req.query,
        params: req.params,
        body: req.body
      };
    ...
    // 保持最近10000条记录
    if (this.auditRecords.length > 10000) {
      this.auditRecords.splice(0, this.auditRecords.length - 10000);
    }
  }
  ...
  // 辅助方法...
  private async checkUserRole(userId: string, roleName: string): Promise<boolean> {
    // 简化实现，实际应该查询用户角色
    return true;
  }

  private extractResourceId(context: PermissionContext): string | undefined {
    // 从URL参数中提取资源ID
