...
export class ApiSecurityMiddleware {
   ...
   * 返回默认的API安全配置
   */
  private getDefaultConfig(): ApiSecurityConfig {
    return {
      enabled: true,
      jwt: {
  ...
  public createSecurityStack() {
    ...
    if (this.config.inputValidation.enabled) {
      middlewares.push(this.validateInput.bind(this));
    }

    // 6. API密钥验证
    if (this.config.apiKey.enabled) {
      middlewares.push(this.validateApiKey.bind(this));
    }
    ...
    if (this.config.audit.enabled) {
      middlewares.push(this.auditRequest.bind(this));
    }
   ...
   * 为每个请求创建安全上下文
   */
  private initializeRequestContext(req: Request, res: Response, next: NextFunction) {
    ...
    if (this.isSuspiciousIp(clientIp)) {
      context.securityFlags.suspicious = true;
   ...
   * 验证和清理请求输入
   */
  private async validateInput(req: Request, res: Response, next: NextFunction) {
    try {
      const context = (req as any).securityContext as ApiRequestContext;
      ...
      for (const [key, value] of Object.entries(req.query)) {
        if (typeof value === 'string' && value.length > this.config.inputValidation.maxParameterLength) {
          context.securityFlags.blocked = true;
      ...
      // 使用增强输入验证器
      if (req.body) {
        const validationResult = await enhancedInputValidator.validateInput(req.body, {
   ...
   * 验证API密钥的有效性和权限
   */
  private validateApiKey(req: Request, res: Response, next: NextFunction) {
    const context = (req as any).securityContext as ApiRequestContext;
    ...
    // 验证API密钥
    if (!this.config.apiKey.validKeys.includes(apiKey)) {
      context.securityFlags.blocked = true;
   ...
   * 检测和防护各种安全威胁
   */
  private detectThreats(req: Request, res: Response, next: NextFunction) {
    const context = (req as any).securityContext as ApiRequestContext;
   ...
   * 记录请求的安全审计日志
   */
  private auditRequest(req: Request, res: Response, next: NextFunction) {
    const context = (req as any).securityContext as ApiRequestContext;
    ...
    res.on('finish', () => {
      ...
      // 确定结果状态
      if (context.securityFlags.blocked) {
        auditLog.result = 'blocked';
      } else if (context.securityFlags.rateLimited) {
        auditLog.result = 'rate_limited';
  ...
  private logSecurityEvent(
    ...
    if (auditLog) {
      auditLog.securityEvents.push(event);
    }

    // 更新可疑IP统计
    if (severity === 'high' || severity === 'critical') {
      const suspiciousInfo = this.suspiciousIps.get(context.clientIp) || { count: 0, lastSeen: 0 };
    ...
    // 报告给全局错误监控
    if (severity === 'high' || severity === 'critical') {
      this.globalErrorMonitor.reportError(new Error(`Security event: ${description}`), {
        type: 'security_event',
        severity,
   ...
   * 保存安全审计日志
   */
  private saveAuditLog(auditLog: SecurityAuditLog) {
    ...
    // 这里可以集成外部日志系统
    if (this.config.audit.logLevel === 'debug' || auditLog.securityEvents.length > 0) {
   ...
   * 统一处理中间件中的错误
   */
  private handleMiddlewareError(error: any, req: Request, res: Response, next: NextFunction) {
    const context = (req as any).securityContext as ApiRequestContext;
   ...
   * 更新安全中间件配置
   */
  public updateConfig(newConfig: Partial<ApiSecurityConfig>) {
 ...
 * 创建并配置API安全中间件实例
 */
export function createApiSecurityMiddleware(config?: Partial<ApiSecurityConfig>) {
  const middleware = ApiSecurityMiddleware.getInstance();
  if (config) {
 ...
 * 获取API安全中间件实例
 */
export function getApiSecurityMiddleware() {
  return ApiSecurityMiddleware.getInstance();
}
