...
export class ApiSecurityMiddleware  { private static instance: ApiSecurityMiddleware;
  ...
  private apiKeyCache: Map<string, { keyId: string; scope: string[]; lastUsed: number  
}> = new Map();
...
private detectPathTraversal(req: Request): boolean  { const pathTraversalPatterns = [;
  ...
  * 记录安全相关事件
  ...
  private logSecurityEvent(
    ...
    // 报告给全局错误监控
    if (severity === 'high' || severity === 'critical') { this.globalErrorMonitor.reportError(new Error(`Security event: ${description 
}`), { type: 'security_event',
        severity,
