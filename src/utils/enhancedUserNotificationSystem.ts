 ...
 * 增强的用户通知系统类
 */
export class EnhancedUserNotificationSystem {
  private static instance: EnhancedUserNotificationSystem;
   ...
   */
  public closeAll(type?: NotificationType): void {
   ...
   * @param config - 新的配置
   */
  public update(id: string, config: Partial<NotificationConfig>): void {
    const notification = this.notifications.get(id);
    if (notification) {
   ...
   * @param id - 通知ID
   */
  public get(id: string): NotificationInstance | undefined {
   ...
   * 获取所有通知
   */
  public getAll(): NotificationInstance[] {
   ...
   * @param position - 通知位置
   */
  public setDefaultPosition(position: NotificationPosition): void {
   ...
   * 渲染通知
   */
  private renderNotification(notification: NotificationInstance): void {
    ...
    if (notification.config.onClick) {
      element.addEventListener('click', notification.config.onClick);
    }
  }
   ...
   * 获取位置容器样式
   */
  private getPositionContainerStyle(position: NotificationPosition): string {
    const baseStyle = `
      position: absolute;
      display: flex;
   ...
   * 更新通知元素
   */
  private updateNotificationElement(notification: NotificationInstance): void {
  ...
  private scheduleAutoClose(notification: NotificationInstance, duration: number): void {
    ...
    if (notification.config.showProgress) {
      this.updateProgress(notification.id, duration);
    }
  }
  ...
  private enforceMaxNotifications(): void {
    ...
    while (activeNotifications.length >= this.maxNotifications) {
      const oldestNotification = activeNotifications.pop();
   ...
   * 持久化通知
   */
  private persistNotification(notification: NotificationInstance): void {
    try {
   ...
   * 移除持久化通知
   */
  private removePersistentNotification(id: string): void {
    try {
      const persistedNotifications = this.getPersistedNotifications();
   ...
   * 获取持久化通知
   */
  private getPersistedNotifications(): Record<string, any> {
    try {
