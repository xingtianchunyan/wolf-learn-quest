// 角色状态常量
/**
 * ROLE组件
 * ROLE组件的功能描述
 * @param props - 组件属性
 * @returns JSX元素
 */
export const ROLE_STATUS = {
  NORMAL: 1, // 正常状态
  DYING: 2, // 濒死状态
  WEAK: 3, // 虚弱状态
  ELIMINATED: 4, // 淘汰状态
} as const;

export type RoleStatusType = (typeof ROLE_STATUS)[keyof typeof ROLE_STATUS];

// 获取状态名称
/**
 * getRoleStatusName函数
 * 获取数据
 *
 * @param status - status参数
 * @returns void
 */
export const getRoleStatusName = (status: number): string => {
  switch (status) {
    case ROLE_STATUS.NORMAL:
      return '正常';
    case ROLE_STATUS.DYING:
      return '濒死';
    case ROLE_STATUS.WEAK:
      return '虚弱';
    case ROLE_STATUS.ELIMINATED:
      return '淘汰';
    default:
      return '未知';
  }
};

// 获取状态颜色
/**
 * getRoleStatusColor函数
 * 获取数据
 *
 * @param status - status参数
 * @returns void
 */
export const getRoleStatusColor = (status: number): string => {
  switch (status) {
    case ROLE_STATUS.NORMAL:
      return 'text-green-400';
    case ROLE_STATUS.DYING:
      return 'text-yellow-400';
    case ROLE_STATUS.WEAK:
      return 'text-orange-400';
    case ROLE_STATUS.ELIMINATED:
      return 'text-red-400';
    default:
      return 'text-gray-400';
  }
};

// 检查能否进行某项操作
/**
 * canPerformAction函数
 * canPerformAction函数的功能描述
 *
 * @param statusEffects - statusEffects参数
 * @param action - action参数
 * @returns void
 */
export const canPerformAction = (
  statusEffects: any,
  action: string
): boolean => {
  if (!statusEffects || typeof statusEffects !== 'object') {
    return false;
  }
  return statusEffects[action] === true;
};

// 获取技能剩余使用次数
/**
 * getSkillUsesRemaining函数
 * 获取数据
 *
 * @param skillUses - skillUses参数
 * @returns void
 */
export const getSkillUsesRemaining = (skillUses: any): number | 'unlimited' => {
  if (!skillUses || typeof skillUses !== 'object') {
    return 0;
  }

  if (skillUses.unlimited === true) {
    return 'unlimited';
  }

  return skillUses.remaining || 0;
};

// 检查是否可以使用技能
/**
 * canUseSkill函数
 * 自定义Hook
 *
 * @param skillUses - skillUses参数
 * @returns void
 */
export const canUseSkill = (skillUses: any): boolean => {
  const remaining = getSkillUsesRemaining(skillUses);
  return (
    remaining === 'unlimited' ||
    (typeof remaining === 'number' && remaining > 0)
  );
};

// 检查是否是猎人反击状态
/**
 * isHunterRevenge函数
 * isHunterRevenge函数的功能描述
 *
 * @param statusEffects - statusEffects参数
 * @returns void
 */
export const isHunterRevenge = (statusEffects: any): boolean => {
  return statusEffects?.is_hunter_revenge === true;
};

// 获取猎人反击剩余时间（秒）
/**
 * getHunterRevengeTimeLeft函数
 * 获取数据
 *
 * @param statusEffects - statusEffects参数
 * @returns void
 */
export const getHunterRevengeTimeLeft = (statusEffects: any): number => {
  if (
    !statusEffects?.is_hunter_revenge ||
    !statusEffects?.hunter_revenge_end_time
  ) {
    return 0;
  }

  const endTime = new Date(statusEffects.hunter_revenge_end_time);
  const timeLeft = Math.max(
    0,
    Math.floor((endTime.getTime() - Date.now()) / 1000)
  );
  return timeLeft;
};

// 将角色状态数字转换为技能系统可识别的状态名称
/**
 * getStatusNameForSkillSystem函数
 * 获取数据
 *
 * @param status - status参数
 * @returns void
 */
export const getStatusNameForSkillSystem = (status: number): string => {
  switch (status) {
    case ROLE_STATUS.NORMAL:
      return 'normal';
    case ROLE_STATUS.DYING:
      return 'dying';
    case ROLE_STATUS.WEAK:
      return 'weak';
    case ROLE_STATUS.ELIMINATED:
      return 'eliminated';
    default:
      return 'normal';
  }
};

// 检查角色是否可以在当前状态下使用技能（基于新的技能系统）
/**
 * canUseSkillWithCurrentStatus函数
 * 自定义Hook
 * @returns void
 */
export const canUseSkillWithCurrentStatus = (
  roleStatus: number,
  requiredStatuses: string[]
): boolean => {
  const currentStatusName = getStatusNameForSkillSystem(roleStatus);
  return requiredStatuses.includes(currentStatusName);
};
