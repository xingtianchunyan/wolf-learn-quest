...
export class ApiErrorHandlingExample {
   ...
   * 批量 API 调用错误处理
   */
  static async fetchMultipleUsers(userIds: string[]): Promise<any[]> {
    const results = [];
    const errors = [];
    ...
    for (const userId of userIds) {
      try {
        ...
        await improvedErrorSystem.handleError(error, {
          context: `batchFetchUsers:${userId}`,
    ...
    // 如果有错误，显示汇总信息
    if (errors.length > 0) {
      userNotificationSystem.showWarning(
...
export class FormValidationExample {
   ...
   * 表单验证
   */
  private static validateForm(formData: Record<string, any>): Error[] {
    const errors: Error[] = [];
    
    if (!formData.email) {
      errors.push(new AppError('邮箱地址不能为空', ErrorCode.DATA_INVALID));
    } else if (!this.isValidEmail(formData.email)) {
      errors.push(new AppError('邮箱地址格式不正确', ErrorCode.DATA_INVALID));
    }
    
    if (!formData.password) {
      errors.push(new AppError('密码不能为空', ErrorCode.DATA_INVALID));
    } else if (formData.password.length < 6) {
      errors.push(new AppError('密码长度至少6位', ErrorCode.DATA_INVALID));
    }
...
export class GameOperationExample {
  ...
  static async useSkill(skillId: string, targetId?: string): Promise<boolean> {
    ...
    try {
      ...
      userNotificationSystem.updateLoadingToError(loadingToast, '技能使用失败');
   ...
   * 投票操作错误处理
   */
  static async submitVote(targetId: string): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.code === 'VOTE_PHASE_ENDED') {
          userNotificationSystem.showInfo('投票阶段已结束');
          return false;
        }
...
export class FileUploadExample {
  ...
  static async uploadFile(file: File): Promise<string | null> {
    ...
    try {
      ...
      userNotificationSystem.updateLoadingToError(loadingToast, '文件上传失败');
    ...
    try {
      ...
      if (options.fallback) {
        try {
          ...
          await improvedErrorSystem.handleError(fallbackError, {
  ...
  static async processBatch<T, R>(
    ...
    for (const item of items) {
      try {
        ...
        await improvedErrorSystem.handleError(error, {
          context: options.context || 'batchProcessing',
          severity: ErrorSeverity.LOW,
          showToUser: false,
          logError: true
        });
        ...
        // 如果错误太多，停止处理
        if (errors.length >= maxErrors && !options.continueOnError) {
          userNotificationSystem.showError(
    ...
    if (errors.length > 0) {
      ...
      userNotificationSystem.showSuccess(
