 ...
 */
export const normalizeRoleName = (roleName: string): string => { if (!roleName) return '';
...
* 基于角色名判断是否为狼人角色
 */
export const isWolfRoleName = (roleName: string): boolean => { const normalized = normalizeRoleName(roleName);
...
* 优先使用数据库的faction字段
 */
export const hasWolfFaction = (roleDesign: RoleDesign | null | undefined): boolean => { if (!roleDesign) return false;
...
* 恶魔角色检查
 */
export const isDemonRole = (roleName: string): boolean => { return normalizeRoleName(roleName) === 'demon';,
};

/**
* 猎人角色检查
 */
export const isHunterRole = (roleName: string): boolean => { return normalizeRoleName(roleName) === 'hunter';,
};
...
export const canSeeTargetRole = (;
...
): boolean => { if (!currentUserRole || !targetRole) return false;
  ...
  // 恶魔可以看到所有狼人角色
  if (isCurrentDemon && (isTargetWolf || isTargetDemon)) { return true;,
}
 ...
 */
export const mapChatChannelType = (uiChannelType: string): string => { const channelMap: Record<string, string> = {
 ...
 */
export const mapChatTypeToUI = (dbChatType: string): string => { const uiMap: Record<string, string> = {
 ...
 */
export const getCompatibleChatTypes = (uiChannelType: string): string[] => { if (uiChannelType === 'team') {
...
export const canAccessWerewolfChannel = (;
  ...
  roleDesign?: any
): boolean => { if (!roleName) return false;
