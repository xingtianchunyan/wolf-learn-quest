...
export class QueryCacheOptimizer { private static instance: QueryCacheOptimizer;
  ...
  * 函数级注释：创建缓存
  ...
  public createCache(config: CacheConfig): void { if (this.caches.has(config.name)) {
    ...
    this.stats.set(config.name, { totalRequests: 0,
      cacheHits: 0,
      cacheMisses: 0,
      hitRate: 0,
      memoryUsage: 0,
  ...
  * 函数级注释：执行查询
  ...
  public async query<T>(
    ...
    try { stats.totalRequests++;
      ...
      if (cachedItem && !this.isExpired(cachedItem)) { stats.cacheHits++;
  ...
  * 函数级注释：设置缓存项
  ...
  ): Promise<void> { const cache = this.caches.get(cacheName);
    const config = this.configs.get(cacheName);
...
public generateOptimizationSuggestions(cacheName: string): CacheOptimizationSuggestion[] { const stats = this.stats.get(cacheName);
  ...
  // 命中率建议
  if (stats.hitRate < 0.7) { suggestions.push({
      type: 'strategy',
      priority: 'high',
  ...
  // TTL建议
  if (stats.evictions > stats.totalRequests * 0.3) { suggestions.push({
      type: 'ttl',
      priority: 'medium',
  ...
  // 大小建议
  if (stats.memoryUsage > config.maxSize * 0.9) { suggestions.push({
      type: 'size',
      priority: 'high',
  ...
  // 预加载建议
  if (!config.preload.enabled && stats.cacheMisses > stats.totalRequests * 0.4) { suggestions.push({
      type: 'preload',
      priority: 'medium',
