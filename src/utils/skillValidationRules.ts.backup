...
export const SKILL_VALIDATION_RULES: Record<string, SkillValidationRule> = { werewolf_attack: {
  ...
  night_attack: { skillName: 'night_attack',
    phases: [3], // 夜晚阶段
    maxUsesPerRound: 1,
    requiresTarget: true,
    targetValidation: async (targetId, gameStateId) => {
  ...
  seer_divination: { skillName: 'seer_divination',
    phases: [3], // 夜晚阶段
    maxUsesPerRound: 1,
    requiresTarget: true,
    targetValidation: async (targetId, gameStateId) => {
  ...
  magic_potion: { skillName: 'magic_potion',
    phases: [3], // 夜晚阶段
  ...
  hunter_revenge: { skillName: 'hunter_revenge',
...
dying_shot: { skillName: 'dying_shot',
  phases: [1, 2, 3, 4], // 任何阶段（被淘汰时触发）
  maxUsesPerGame: 1,
  requiresTarget: true,
  customValidation: async (userId, gameStateId, skillData) => {
...
self_destruct: { skillName: 'self_destruct',
  phases: [3], // 夜晚阶段
  maxUsesPerGame: 1,
  requiresTarget: true,
  customValidation: async (userId, gameStateId, skillData) => {
...
export const validateSkillUnified = async (;
  ...
  // 验证使用次数限制
  if (rule.maxUsesPerGame || rule.maxUsesPerRound) { const usageValid = await validateSkillUsageLimits(;
      skillName,
      userId,
      gameStateId,
      rule.maxUsesPerGame,
  ...
  // 自定义验证
  if (rule.customValidation) { const customValid = await rule.customValidation(;
      userId,
      gameStateId,
      skillData
    );
