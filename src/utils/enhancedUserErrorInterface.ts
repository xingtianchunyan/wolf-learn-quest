...
* 定义错误界面的展示配置
 */
export interface ErrorDisplayConfig { /** 展示模式  */
...
* 定义用户友好的错误信息结构
 */
export interface UserFriendlyErrorInfo { /** 错误ID  */
  id: string;
  /** 用户友好标题  */
  title: string;
  /** 简短描述  */
...
* 定义错误的解决方案信息
 */
export interface ErrorSolution { /** 解决方案ID  */
  id: string;
  /** 解决方案标题  */
  title: string;
  /** 解决方案描述  */
...
* 定义相关帮助资源的链接
 */
export interface HelpLink { /** 链接标题  */
  title: string;
  /** 链接URL  */
  url: string;
  /** 链接类型  */
...
* 定义用户反馈的结构
 */
export interface ErrorFeedback { /** 错误ID  */
  errorId: string;
  /** 反馈类型  */
...
* - 多模态交互支持
 */
export class EnhancedUserErrorInterface { private static instance: EnhancedUserErrorInterface;
  ...
  * 实现单例模式
   */
  public static getInstance(config?: Partial<ErrorDisplayConfig>): EnhancedUserErrorInterface { if (!EnhancedUserErrorInterface.instance) {
  ...
  * 将技术错误转换为用户友好的信息
  ...
  public generateUserFriendlyError(
    error: any,
    classification: ErrorClassification
  ): UserFriendlyErrorInfo { const errorId = this.generateErrorId();
  ...
  * 将技术错误映射为用户友好的描述
   */
  private mapErrorToFriendlyInfo(error: any, classification: ErrorClassification): { title: string;
    description: string;
    details?: string;,
  ...
  * 根据错误类型生成可能的解决方案
   */
  private generateSolutions(error: any, classification: ErrorClassification): ErrorSolution[] { const solutions: ErrorSolution[] = [];
    const category = classification.category;
    ...
    switch (category) {
      ...
      case 'skill':
      solutions.push({ id: 'check_skill_conditions',
        title: '检查技能使用条件',
    ...
    // 根据成功率排序
    return solutions.sort((a, b) => { if (a.recommended && !b.recommended) return -1;
  ...
  * 根据错误类型生成相关帮助资源
   */
  private generateHelpLinks(error: any, classification: ErrorClassification): HelpLink[] { const links: HelpLink[] = [];
    const category = classification.category;
  ...
  * 根据错误分类估算解决时间
   */
  private estimateResolutionTime(classification: ErrorClassification): string { switch (classification.severity) {
  ...
  * 根据错误分类确定影响范围
   */
  private determineImpactScope(classification: ErrorClassification): 'user' | 'feature' | 'system' | 'global' { switch (classification.category) {
      case 'validation':
      case 'permission':
      return 'user';
      case 'skill':
  ...
  * 根据严重级别返回对应图标
   */
  private getErrorIcon(severity: ErrorSeverity): React.ReactNode { switch (severity) {
      case ErrorSeverity.LOW:
  ...
  * 根据严重级别返回对应颜色主题
   */
  private getColorTheme(severity: ErrorSeverity): string { switch (severity) {
      case ErrorSeverity.LOW:
      return 'blue';
      case ErrorSeverity.MEDIUM:
      return 'yellow';
  ...
  * 收集用户对错误处理的反馈
   */
  public async collectFeedback(feedback: Omit<ErrorFeedback, 'timestamp' | 'userAgent' | 'pageUrl'>): Promise<void> { const completeFeedback: ErrorFeedback = {
      ...feedback,
      timestamp: Date.now(),
  ...
  * 根据用户反馈更新解决方案的成功率
   */
  public updateSolutionSuccessRate(solutionId: string, success: boolean): void { const currentRate = this.solutionSuccessRates.get(solutionId) || 0.5;
  ...
  * 动态更新显示配置
   */
  public updateConfig(newConfig: Partial<ErrorDisplayConfig>): void { this.config = { ...this.config, ...newConfig  };,
}
  ...
  * 获取用户反馈历史记录
   */
  public getFeedbackHistory(): ErrorFeedback[] { return [...this.feedbackHistory];,
}
  ...
  * 获取解决方案成功率统计
   */
  public getSolutionStats(): Map<string, number> { return new Map(this.solutionSuccessRates);,
}
}
 ...
 */
export function useEnhancedUserErrorInterface(config?: Partial<ErrorDisplayConfig>) { const [interface] = useState(() => EnhancedUserErrorInterface.getInstance(config));
  ...
  const collectFeedback = useCallback((feedback: Omit<ErrorFeedback, 'timestamp' | 'userAgent' | 'pageUrl'>) => { return interface.collectFeedback(feedback);,
}, [interface]);

  const updateSolutionSuccess = useCallback((solutionId: string, success: boolean) => { interface.updateSolutionSuccessRate(solutionId, success);,
}, [interface]);
