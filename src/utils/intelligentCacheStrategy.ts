...
class IntelligentCacheStrategy {
  ...
  public async getBatch<T>(
    ...
    if (criticalAndHigh.length > 0) {
      const highPriorityResults =
        await this.processBatchRequests(criticalAndHigh);
    ...
    if (mediumRequests.length > 0) {
      const mediumResults =
        await this.processBatchRequestsInChunks(mediumRequests);
    ...
    if (lowPriorityRequests.length > 0) {
      ...
      setTimeout(async () => {
        const lowResults =
          await this.processBatchRequestsInChunks(lowPriorityRequests);
        // 这些结果不返回给调用者，但会被缓存
      }, 100);
    }
  ...
  private async getFromMultiLevelCache<T>(key: string): Promise<T | null> {
    ...
    if (compressedData) {
      ...
      // 提升到L1缓存
      await optimizedQueryCache.set(key, decompressed, {
        ttl: this.config.baseTTL,
      });
      return decompressed;
    }
  ...
  private updateStats(
    ...
    if (isHit) {
      ...
      stats.avgResponseTime =
        (stats.avgResponseTime * (totalRequests - 1) + responseTime) /
        totalRequests;
  ...
  private calculateAccessTrend(accessHistory: number[]): number {
    ...
    const olderAccesses = accessHistory.filter(
      time => time > twoHoursAgo && time <= oneHourAgo
    ).length;
  ...
  private shouldBackgroundRefresh(key: string): boolean {
    ...
    // 高频访问的数据需要更频繁的后台刷新
    if (
      stats.pattern === QueryPattern.FREQUENT &&
      timeSinceLastAccess > 2 * 60 * 1000
    ) {
      return true;
    }
    ...
    // 可预测模式的数据可以提前刷新
    if (
      stats.pattern === QueryPattern.PREDICTABLE &&
      timeSinceLastAccess > 5 * 60 * 1000
    ) {
      return true;
    }
  ...
  private updateGlobalMetrics(): void {
    ...
    if (totalQueries > 0) {
      this.performanceMetrics.hitRate =
        this.performanceMetrics.cacheHits / totalQueries;
    }
    ...
    this.queryStats.forEach(stats => {
      totalResponseTime +=
        stats.avgResponseTime * (stats.hitCount + stats.missCount);
    ...
    // 计算内存使用
    this.performanceMetrics.memoryUsage = Array.from(
      this.queryStats.values()
  ...
  private performAnalysisAndOptimization(): void {
    ...
    // 清理过期统计
    this.queryStats.forEach((stats, key) => {
      if (now - stats.lastAccess > 2 * 60 * 60 * 1000) {
        // 2小时未访问
        this.queryStats.delete(key);
        this.accessHistory.delete(key);
      }
  ...
  public getStats() {
    ...
    this.queryStats.forEach((queryStats, key) => {
      stats.queries[key] = {
        ...queryStats,
        hitRate:
          queryStats.hitCount / (queryStats.hitCount + queryStats.missCount),
      };
    });
...
export function useIntelligentCache<T>(
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const result = await intelligentCacheStrategy.get(
        key,
        fetcherRef.current,
        optionsRef.current
      );
