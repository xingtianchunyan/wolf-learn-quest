...
private async performValidation(data: any, config: ValidationConfig): Promise<ValidationResult> { const result: ValidationResult =  {
    ...
    const context: ValidationContext = { fieldName: '',
      data,
      config,
      path: [],
      depth: 0  
    ...
    // 验证每个字段
    for (const fieldConfig of config.fields) { context.fieldName = fieldConfig.name;
  ...
  private async performSecurityValidation(
    ...
    for (const [path, value] of Object.entries(flattenedData)) {
      if (typeof value === 'string') {
        ...
        // SQL注入检测
        if (config.global?.enableSQLInjectionProtection && this.detectSQLInjection(value)) { result.errors.push({
            field: path,
            type: ValidationRuleType.CUSTOM,
        ...
        // 路径遍历检测
        if (this.detectPathTraversal(value)) { result.errors.push({
            field: path,
            type: ValidationRuleType.CUSTOM,
            message: '检测到路径遍历攻击模式',
            value,
        ...
        // 命令注入检测
        if (this.detectCommandInjection(value)) { result.errors.push({
            field: path,
            type: ValidationRuleType.CUSTOM,
            message: '检测到命令注入攻击模式',
            value,
  ...
  * 验证数据类型并进行必要的转换
  ...
  private async validateAndConvertType(
    ...
    convertedValue: num   
        ...
        if (isNaN(int) || int !== Number(value)) { return {
            convertedValue: value,
            error: {
              field: context.fieldName,
              type: ValidationRuleType.TYPE,
  ...
  * 验证上传的文件
  ...
  private async validateFile(
    ...
    // 检查文件大小
    if (config.maxSize && value.size > config.maxSize) { return {
        error: {
          field: context.fieldName,
          type: rule.type,
    ...
    // 检查文件类型
    if (config.allowedTypes && config.allowedTypes.length > 0) { const fileExtension = this.getFileExtension(value.name || '');
    ...
    // 检查MIME类型
    if (config.allowedMimeTypes && config.allowedMimeTypes.length > 0) { if (!config.allowedMimeTypes.includes(value.type)) {
        return {
          error: {
            field: context.fieldName,
            type: rule.type,
  ...
  * 验证对象及其属性
  ...
  private async validateObject(
    ...
    // 检查必需属性
    if (required && Array.isArray(required)) { for (const requiredProp of required) {
    ...
    // 检查额外属性
    if (additionalProperties === false) { const allowedProps = properties ? Object.keys(properties) : [];
