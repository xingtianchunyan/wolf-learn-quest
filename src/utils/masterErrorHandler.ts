...
* 错误处理选项接口
 */
export interface ErrorHandlingOptions  { /** 是否记录日志 */
  logError?: boolean;
  /** 是否显示用户通知 */
...
* 错误分类接口
 */
export interface ErrorClassification  { /** 错误类别 */
...
* 统一管理所有错误处理逻辑
 */
export class MasterErrorHandler  { private static instance: MasterErrorHandler;
  ...
  private errorStats: ErrorStatistics = {
    ...
    bySeverity: { [ErrorSeverity.LOW]: 0,
 ...
 * 获取单例实例
 */
public static getInstance(): MasterErrorHandler { if (!MasterErrorHandler.instance)  {
  ...
  * @returns 错误处理结果
  ...
  public async handleError(
    ...
    try {
      ...
      // 6. 尝试自动恢复
      let recovered = false;
      if (options.attemptRecovery !== false && unifiedError.retryable) { recovered = await this.attemptRecovery(unifiedError, options)
}

      // 7. 显示用户通知
      if (options.showNotification !== false && !options.silent) { await this.showUserNotification(unifiedError, options, recovered)
}

      // 8. 构建处理结果
      const result: ErrorHandlingResult = { errorId,
        handled: true,
        recovered,
      ...
      // 9. 更新恢复统计
      if (recovered) { this.errorStats.recovered++
      ...
      if (recovered && options.onRecovery) { options.onRecovery(unifiedError)
}
  ...
  * @returns 统一错误对象
  ...
  private normalizeError(
    ...
    context: ErrorContext,
    errorId: string
  ): UnifiedError { let type: UnifiedError['type'] = 'unknown';
...
} else if (error instanceof SkillError) { type = 'skill';
      code = error.type || 'SKILL_ERROR';
      message = error.message;
      severity = this.mapSkillErrorSeverity(error.type);
      retryable = this.isSkillErrorRetryable(error.type)
} else if (error instanceof Error) { if (error.name === 'NetworkError' || error.message.includes('fetch')) {
        type = 'network';
        code = 'NETWORK_ERROR';
        severity = ErrorSeverity.HIGH;
        retryable = true;
        maxRetries = 3
} else if (error.name === 'ValidationError') { type = 'validation';
        code = 'VALIDATION_ERROR';
        severity = ErrorSeverity.MEDIUM;
        retryable = false
} else if (error.name === 'PermissionError') { type = 'permission';
  ...
  * @returns 错误分类
  ...
  private classifyError(
    ...
    message: string,
    severity: ErrorSeverity
  ): ErrorClassification { const categories = {
      network: {
    ...
    const defaultCategory = { category: 'internal' as const,
      userMessage: '发生了未知错误',
  ...
  * @returns 是否可重试
   */
private isErrorRetryable(type: string, code: string): boolean  { const retryableTypes = ['network'];
  ...
  * @returns 严重级别
   */
private mapAppErrorSeverity(code?: string): ErrorSeverity { const severityMap: Record<string, ErrorSeverity> =  {
  ...
  * @returns 是否可重试
   */
private isAppErrorRetryable(code?: string): boolean  { const retryableCodes = [;
  ...
  * @returns 严重级别
   */
private mapSkillErrorSeverity(type?: SkillErrorType): ErrorSeverity { const severityMap: Record<SkillErrorType, ErrorSeverity> =  {
...
private isSkillErrorRetryable(type?: SkillErrorType): boolean  { const retryableTypes = [;
  ...
  * @returns 是否恢复成功
  ...
  private async attemptRecovery(
    error: UnifiedError,
    options: ErrorHandlingOptions
  ): Promise<boolean> { if (!error.retryable) {
      return false
}
    ...
    if (currentRetries >= maxRetries) { logger.warn('达到最大重试次数', { errorId: error.id, retries: currentRetries  
});
      return false
}
    ...
    logger.info('尝试自动恢复', { errorId: error.id,
    ...
    } catch (recoveryError) { logger.error('恢复过程中发生错误', { errorId: error.id, recoveryError  });
      return false
}
  }
  ...
  * @returns 是否恢复成功
   */
private async recoverNetworkError(error: UnifiedError): Promise<boolean>  { // 简单的网络连接测试
    try {
  ...
  * @returns 是否恢复成功
   */
private async recoverSkillError(error: UnifiedError): Promise<boolean>  { // 技能错误通常需要重新验证状态
  ...
  * @returns 是否恢复成功
   */
private async recoverAppError(error: UnifiedError): Promise<boolean>  { // 应用错误的恢复策略
  ...
  * @param recovered - 是否已恢复
  ...
  private async showUserNotification(
    ...
    options: ErrorHandlingOptions,
    recovered: boolean
  ): Promise<void> { const message = options.customMessage || this.getUserFriendlyMessage(error, options);
  ...
  * @returns 通知类型
   */
private getNotificationType(severity: ErrorSeverity, recovered: boolean): string { if (recovered)  {
      return 'success'
}

    switch (severity) { case ErrorSeverity.LOW:
      return 'info';
  ...
  * @returns 用户友好消息
  ...
  private getUserFriendlyMessage(
    error: UnifiedError,
    options: ErrorHandlingOptions
  ): string { if (options.customMessage) {
  ...
  * @returns 恢复建议
   */
private getRecoverySuggestion(error: UnifiedError): string  { return error.classification.suggestedAction || '请重试或联系技术支持'
}
...
private logError(error: UnifiedError): void { const logData =  {
    ...
    switch (error.severity) { case ErrorSeverity.LOW:
  ...
  * @param error - 错误对象
   */
private async reportToMonitoring(error: UnifiedError): Promise<void> { try  {
      // 这里应该调用实际的监控系统API
      // 暂时使用日志记录
...
} catch (reportError) { logger.error('上报监控系统失败', reportError)
}
  }
  ...
  * @param error - 错误对象
   */
private addToHistory(error: UnifiedError): void  { this.errorHistory.unshift(error);
  ...
  * @param error - 错误对象
   */
private updateStats(error: UnifiedError): void  { this.errorStats.total++;
...
private generateErrorId(): string { return `error_${Date.now() 
  ...
  * @returns 错误统计
   */
public getStatistics(): ErrorStatistics { return  { ...this.errorStats  
}
}
  ...
  * @returns 错误历史
   */
public getErrorHistory(limit: number = 50): UnifiedError[]  { return this.errorHistory.slice(0, limit)
}
  ...
  * @param maxAge - 最大保留时间（毫秒）
   */
public cleanupExpiredErrors(maxAge: number = 24 * 60 * 60 * 1000): void  { const cutoffTime = Date.now() - maxAge;
 ...
 * 重置统计信息
 */
public resetStatistics(): void { this.errorStats =  {
      total: 0,
      handled: 0,
      recovered: 0,
      failed: 0,
