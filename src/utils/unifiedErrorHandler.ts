...
export class UnifiedErrorHandler {
  ...
  private async executeHandlingStrategy(
    ...
    const showMessage = options.showUserMessage !== false;
   ...
   * @param error - 错误对象
   */
  private redirectToErrorPage(error: UnifiedError): void {
    const errorPageUrl = `/error?code=${error.code}&id=${error.id}`;
   ...
   * @param error - 统一错误对象
   */
  private logUnifiedError(error: UnifiedError): void {
    const logData = {
      id: error.id,
      type: error.type,
      code: error.code,
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]): Promise<any> {
      try {
        ...
        await unifiedErrorHandler.handleError(error, context, options);
        throw error;
      }
    };
