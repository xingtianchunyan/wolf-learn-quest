...
* 统一错误接口
 */
export interface UnifiedErrorData { /** 错误唯一标识  */
  id: string;
  /** 错误类型  */
 ...
 */
export class UnifiedErrorManager { private static instance: UnifiedErrorManager;
  ...
  * 获取单例实例
   */
  public static getInstance(): UnifiedErrorManager { if (!UnifiedErrorManager.instance) {
  ...
  * @returns 错误处理结果
  ...
  public async handleError(
    ...
    try {
      ...
      // 5. 上报监控
      if (options.reportToMonitoring !== false) { await this.reportToMonitoring(unifiedError);,
}
      ...
      logger.debug('错误处理完成', { errorId: unifiedError.id,
        processingTime,
  ...
  * @param options - 重试选项
   */
  public async retry<T>(errorId: string,
    operation: () => Promise<T>,
    options: { maxRetries?: number; delay?: number  } = {}
  ): Promise<T | null> { const error = this.errorHistory.find(e => e.id === errorId);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) { try {
        ...
        return result;,
} catch (retryError) { error.currentRetries = attempt;
  ...
  * 获取错误统计信息
   */
  public getErrorStats() { const now = Date.now();
    ...
    const bySeverity = this.errorHistory.reduce((acc, error) => { acc[error.severity] = (acc[error.severity] || 0) + 1;
      return acc;,
  ...
  * @param severity - 过滤严重级别
   */
  public getErrorHistory(limit = 50, severity?: ErrorSeverity): UnifiedErrorData[] { let filtered = this.errorHistory;
  ...
  * @param olderThan - 清理指定时间之前的记录（毫秒）
   */
  public cleanupHistory(olderThan = 24 * 60 * 60 * 1000): void { const cutoff = Date.now() - olderThan;
  ...
  * @param context - 错误上下文
   */
  private normalizeError(error: any, context: ErrorContext): UnifiedErrorData { const errorId = this.generateErrorId();
    ...
    if (error instanceof SkillError) { return {
        id: errorId,
        type: 'skill',
        code: error.type,
        message: error.message,
  ...
  * 使用分类结果丰富错误信息
  ...
  ): void { error.severity = classification.severity;
  ...
  * 执行错误处理策略
  ...
  private async executeHandlingStrategy(
    error: UnifiedErrorData,
    options: ErrorHandlingOptions
  ): Promise<ErrorHandlingResult> { const strategy = options.recoveryStrategy ?;
    ...
    switch (strategy) {
      ...
      if (options.showNotification !== false) { await this.showModal(error);,
}
      break;
      ...
      if (options.autoRetry !== false && error.retryable) { await this.scheduleRetry(error, options);,
}
      break;
  ...
  * 显示通知
  ...
  ): Promise<void> { await this.notificationSystem.show({
      type,
  ...
  * 显示模态框
   */
  private async showModal(error: UnifiedErrorData): Promise<void> { // 这里可以集成模态框组件
  ...
  * 处理重定向
   */
  private async handleRedirect(error: UnifiedErrorData): Promise<void> { if (error.code === 'AUTH_REQUIRED') {
      // 重定向到登录页面
      window.location.href = '/login';,
} else if (error.code === 'PERMISSION_DENIED') { // 重定向到无权限页面
  ...
  * 安排重试
  ...
  private async scheduleRetry(
    error: UnifiedErrorData,
    options: ErrorHandlingOptions
  ): Promise<void> { const delay = options.retryDelay || this.DEFAULT_RETRY_DELAY;
  ...
  * 执行备用方案
   */
  private async executeFallback(error: UnifiedErrorData): Promise<void> { logger.info('执行备用方案', { errorId: error.id  });
    // 这里可以实现具体的备用方案逻辑,
}
  ...
  * 记录错误到历史
   */
  private addToHistory(error: UnifiedErrorData): void { this.errorHistory.unshift(error);
  ...
  * 记录错误日志
   */
  private logError(error: UnifiedErrorData): void { const logData = {
      errorId: error.id,
      type: error.type,
      code: error.code,
      message: error.message,
  ...
  * 上报到监控服务
   */
  private async reportToMonitoring(error: UnifiedErrorData): Promise<void> { try {
...
} catch (reportError) { logger.error('上报错误监控失败', reportError);,
}
  }
  ...
  * 清理重试计时器
   */
  private clearRetryTimer(errorId: string): void { const timer = this.retryTimers.get(errorId);
    if (timer) {
      clearTimeout(timer);
  ...
  // 辅助方法
  private mapErrorCodeToSeverity(code: string): ErrorSeverity { const criticalCodes = ['SYSTEM_ERROR', 'DATABASE_ERROR'];
  ...
  private isRetryable(code: string): boolean { const retryableCodes = ['NETWORK_ERROR', 'TIMEOUT_ERROR', 'SERVER_ERROR'];
    return retryableCodes.includes(code);,
}

  private getActionSuggestion(code: string): string { const suggestions: Record<string, string> = {
  ...
  private mapSkillErrorToSeverity(type: SkillErrorType): ErrorSeverity { switch (type) {
  ...
  private isSkillErrorRetryable(type: SkillErrorType): boolean { const retryableTypes = [SkillErrorType.COOLDOWN_ACTIVE, SkillErrorType.INSUFFICIENT_RESOURCES];
    return retryableTypes.includes(type);,
}

  private getSkillErrorUserMessage(type: SkillErrorType): string { const messages: Record<SkillErrorType, string> = {
  ...
  private getSkillErrorActionSuggestion(type: SkillErrorType): string { const suggestions: Record<SkillErrorType, string> = {
  ...
  private isNetworkError(error: any): boolean { return error?.name === 'NetworkError' ||;
    error?.code === 'NETWORK_ERROR' ||;
    error?.message?.includes('fetch') ||
    error?.message?.includes('network');,
}

  private isValidationError(error: any): boolean { return error?.name === 'ValidationError' ||;
  ...
  ): ErrorHandlingStrategy { switch (strategy) {
  ...
  private getNotificationTitle(severity: ErrorSeverity): string { switch (severity) {
  ...
  private getNotificationDuration(severity: ErrorSeverity): number { switch (severity) {
