...
import { UnifiedErrorHandler  } from
  unifiedErrorHandler,
  ErrorSeverity,
  ErrorHandlingStrategy,
  withUnifiedErrorHandling,
  createErrorWrapper,
  type ErrorContext  } from '../unifiedErrorHandler';
...
it('应该正确处理 AppError', async () =>  {
      ...
      const context: ErrorContext = { userId: 'test-user',
 ...
 * 测试 SkillError 处理
 */
it('应该正确处理 SkillError', async () => { const skillError: SkillError =  {
...
it('应该正确应用错误处理装饰器功能', async () =>  {
      ...
      const descriptor = { value: instance.testMethod,
        writable: true,
        enumerable: true,
        configurable: true  
};
