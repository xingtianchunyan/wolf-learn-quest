...
export class SecurityMiddleware {
  ...
  public async performSecurityCheck(
    ...
    try {
      ...
      // 4. 权限检查
      if (options.requiredPermissions && options.requiredPermissions.length > 0 && context.userId) {
      ...
      // 5. CSRF保护检查
      if (options.enableCSRFProtection && context.method !== 'GET') {
        const csrfToken = context.headers['x-csrf-token'] || context.headers['csrf-token'];
        ...
        if (context.userId && context.sessionId) {
          const csrfResult = securityEnhancement.validateCSRFToken(
            csrfToken,
            context.userId,
            context.sessionId
          );
      ...
      // 6. 输入验证
      if (options.enableInputValidation && context.body) {
        const validationResult = enhancedInputValidator.validate(context.body, {}, {
        ...
        if (validationResult.securityWarnings.length > 0) {
          result.warnings.push(...validationResult.securityWarnings);
          result.metadata.securityWarnings = validationResult.securityWarnings;
        }
      }

      // 7. XSS保护检查
      if (options.enableXSSProtection && context.body) {
        const xssCheck = this.checkForXSS(context.body);
      ...
      // 8. SQL注入保护检查
      if (options.enableSQLInjectionProtection && context.body) {
        const sqlCheck = securityEnhancement.detectSQLInjection(JSON.stringify(context.body));
      ...
      if (options.customValidation) {
        const customResult = await options.customValidation(context);
   ...
   * 创建Express中间件
   */
  public createExpressMiddleware(options: SecurityMiddlewareOptions) {
    return async (req: any, res: any, next: any) => {
      try {
        ...
        logger.error('Express安全中间件错误', error);
        res.status(500).json({
   ...
   */
  public createSupabaseMiddleware(options: SecurityMiddlewareOptions) {
    return async (req: Request): Promise<{ passed: boolean; response?: Response }> => {
      try {
        ...
        logger.error('Supabase安全中间件错误', error);
        return {
          passed: false,
          response: new Response(JSON.stringify({
            error: 'Internal security error'
