...
export class SecurityMiddleware {
  ...
  public async performSecurityCheck(
    ...
    try {
      ...
      if (
        context.ipAddress &&
        securityEnhancement.isIPBlocked(context.ipAddress)
      ) {
        result.passed = false;
      ...
      if (
        ...
        for (const permission of options.requiredPermissions) {
          const permissionResult =
            await enhancedPermissionSystem.checkPermission(permission, {
      ...
      if (options.enableCSRFProtection && context.method !== 'GET') {
        const csrfToken =
          context.headers['x-csrf-token'] || context.headers['csrf-token'];
      ...
      if (options.enableSQLInjectionProtection && context.body) {
        const sqlCheck = securityEnhancement.detectSQLInjection(
      ...
      if (result.passed) {
        logger.debug('安全检查通过', {
          path: context.path,
          userId: context.userId,
          checks: Object.keys(options).filter(
            key => options[key as keyof SecurityMiddlewareOptions]
          ),
        });
      }
...
export const createSupabaseSecurityMiddleware = (
  options: SecurityMiddlewareOptions
) => {
