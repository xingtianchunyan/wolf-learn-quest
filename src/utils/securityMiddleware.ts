...
* 安全中间件选项接口
 */
export interface SecurityMiddlewareOptions { requireAuth?: boolean;
...
export class SecurityMiddleware { private static instance: SecurityMiddleware;
  ...
  * 检查XSS攻击
   */
  private checkForXSS(data: any): { detected: boolean; patterns: string[]  } { const dataString = JSON.stringify(data).toLowerCase();
    const xssPatterns = [;
      /<script[^>]*>.*?<\/script>/gi,
      /javascript\s*:/gi,
      /on\w+\s*=\s*[''][^'']*["']/gi,
  ...
  * 创建Supabase Edge Function中间件
   */
  public createSupabaseMiddleware(options: SecurityMiddlewareOptions) { return async (req: Request): Promise<{ passed: boolean; response?: Response  }> => { try {
...
// 导出便捷函数
export const createSecurityMiddleware = (options: SecurityMiddlewareOptions) => { return securityMiddleware.createExpressMiddleware(options);,
};

export const createSupabaseSecurityMiddleware = (options: SecurityMiddlewareOptions) => { return securityMiddleware.createSupabaseMiddleware(options);,
};