...
export function checkNightSkillRestriction(
  ...
  const isWitch =
    roleNameLower.includes('witch') || roleNameLower.includes('女巫');
  ...
  const hasUsedInCurrentNight = userSkillUses.some(
    use => use.phase === 'night'
  );
...
export function checkSkillSpecificRestrictions(
  ...
  if (
    ...
    if (potionType === 'protection') {
      const protectionUses = userSkillUses.filter(
        use =>
          use.skill_name === 'magic_potion' &&
          (use as SkillUseRecord & { potionType?: string }).potionType ===
            'protection'
      );
    ...
    if (potionType === 'attack') {
      const attackUses = userSkillUses.filter(
        use =>
          use.skill_name === 'magic_potion' &&
          (use as SkillUseRecord & { potionType?: string }).potionType ===
            'attack'
  ...
  if (
    ...
    (roleNameLower.includes('hunter') || roleNameLower.includes('猎人'))
  ) {
    const shotUses = userSkillUses.filter(
      use => use.skill_name === 'dying_shot'
    );
  ...
  if (
    skillName === 'voodoo' &&
    (roleNameLower.includes('warlock') || roleNameLower.includes('暗夜术士'))
  ) {
  ...
  if (
    ...
    (roleNameLower.includes('white') || roleNameLower.includes('白狼'))
  ) {
    const destructUses = userSkillUses.filter(
      use => use.skill_name === 'self_destruct'
    );
