...
export class EnhancedPermissionSystem {
  private static instance: EnhancedPermissionSystem;
   ...
   */
  public static getInstance(): EnhancedPermissionSystem {
    if (!EnhancedPermissionSystem.instance) {
  ...
  private getContextValue(field: string, context: PermissionContext): any {
    return context.customContext?.[field] || null;
  }
   ...
   * 检查权限检查结果缓存是否有效
   */
  private isCacheValid(result: PermissionCheckResult): boolean {
   ...
   * 检查用户权限缓存是否过期
   */
  private isPermissionCacheExpired(userPermissions: UserPermissions): boolean {
  ...
  private generatePermissionId(name: string): string {
    return `perm_${name.replace(/\./g, '_')}_${Date.now()}`;
  }
  ...
  private cleanupExpiredCache() {
    ...
    // 清理用户权限缓存
    for (const [userId, userPermissions] of this.userPermissions.entries()) {
      if (this.isPermissionCacheExpired(userPermissions)) {
   ...
   * 获取权限审计日志
   */
  public getAuditLogs(limit: number = 100): PermissionAuditLog[] {
 ...
 * 创建并返回权限系统实例
 */
export function createEnhancedPermissionSystem() {
  return EnhancedPermissionSystem.getInstance();
}
