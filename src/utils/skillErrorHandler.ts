...
export class SkillErrorHandler {
   ...
   * 处理技能错误
   */
  async handleError(error: SkillError, context?: any): Promise<void> {
    // 记录错误
    this.logError(error);
  ...
  private logError(error: SkillError): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('[SkillError]', error);
    }
  }
  ...
  private async handleNetworkError(error: SkillError, context?: any): Promise<void> {
    ...
    // 自动重试网络请求
    if (context?.retryFunction && context?.retryCount < 5) {
      setTimeout(() => {
        context.retryFunction();
        context.retryCount = (context.retryCount || 0) + 1;
      }, 2000);
  ...
  private async handleConflictError(error: SkillError, context?: any): Promise<void> {
    ...
    // 刷新游戏状态
    if (context?.refreshGameState) {
      await context.refreshGameState();
    }
  }
