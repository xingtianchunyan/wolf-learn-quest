...
// 检查角色是否可以在当前阶段使用技能
export const canUseSkillInPhase = (
...
// 检查角色是否在可用技能的状态
export const canUseSkillWithStatus = (
  skillEffects: SkillEffects,
  currentStatus: string
): boolean => {
  // 如果没有配置状态要求，默认只有正常状态可以使用技能
  if (
    !skillEffects?.required_status ||
    skillEffects.required_status.length === 0
  ) {
...
export const getSkillEffectTypes = (skillEffects: SkillEffects): string[] => {
  ...
  return Array.isArray(skillEffects.effect_type)
    ? skillEffects.effect_type
    : [skillEffects.effect_type];
};
...
export const canUseSkillInGameState = (
  ...
  return (
    canUseSkillInPhase(skillEffects, phaseName, skillName) &&
