...
export class UnifiedErrorSystem {
   ...
   * 获取单例实例
   */
  public static getInstance(): UnifiedErrorSystem {
      ...
      try {
        ...
        if (handlingResult.shouldRetry && options.customRecovery) {
          try {
            ...
            logger.warn('自定义恢复失败', { recoveryError });
          }
        }
   ...
   * @param options - 处理选项
   */
  private applyOptionsOverride(error: UnifiedError, options: ErrorHandlingOptions): void {
    if (options.strategyOverride) {
      error.strategy = options.strategyOverride;
    }
    
    if (options.severityOverride) {
      error.severity = options.severityOverride;
    }
    ...
    if (options.customMessage) {
      error.userMessage = options.customMessage;
    }
  }
    ...
    try {
      ...
      switch (error.strategy) {
        ...
        case ErrorHandlingStrategy.MODAL:
          if (options.showNotification !== false) {
            await this.showModal(error.userMessage, error);
          }
          result.handled = true;
        ...
        case ErrorHandlingStrategy.RETRY:
          if (error.retryable && options.attemptRecovery !== false) {
      ...
      if (error.recoverySuggestion) {
        result.recoverySuggestion = error.recoverySuggestion;
      }

      return result;

    } catch (strategyError) {
      logger.error('执行错误处理策略失败', {
   ...
   * @param error - 统一错误对象
   */
  private async networkFallback(error: UnifiedError): Promise<boolean> {
    // 尝试使用缓存数据
    if (typeof window !== 'undefined' && 'caches' in window) {
      try {
        ...
        logger.warn('缓存备用方案失败', { cacheError });
      }
    }
    return false;
  }
   ...
   * @param error - 统一错误对象
   */
  private async skillFallback(error: UnifiedError): Promise<boolean> {
    // 重置技能状态或使用默认技能
   ...
   * @param error - 统一错误对象
   */
  private async appFallback(error: UnifiedError): Promise<boolean> {
    // 根据具体的应用错误类型执行不同的备用方案
    switch (error.code) {
   ...
   * @param error - 统一错误对象
   */
  private async genericFallback(error: UnifiedError): Promise<boolean> {
    // 通用的备用方案，如显示默认内容
   ...
   * @param severity - 严重级别
   */
  private async showToast(message: string, severity: ErrorSeverity): Promise<void> {
   ...
   * @param error - 错误对象
   */
  private async showModal(message: string, error: UnifiedError): Promise<void> {
    if (typeof window === 'undefined') return;
   ...
   * @param error - 错误对象
   */
  private async redirectToErrorPage(error: UnifiedError): Promise<void> {
    if (typeof window === 'undefined') return;
   ...
   * @param error - 统一错误对象
   */
  private updateStatistics(error: UnifiedError): void {
  ...
  private calculateRetryDelay(retryCount: number): number {
    return Math.min(this.DEFAULT_RETRY_DELAY * Math.pow(2, retryCount), 30000);
  }
  ...
  private determineStrategy(code: string): ErrorHandlingStrategy {
    const strategyMap: Record<string, ErrorHandlingStrategy> = {
   ...
   * @param error - 技能错误对象
   */
  private getSkillErrorUserMessage(error: SkillError): string {
    const messageMap: Record<SkillErrorType, string> = {
   ...
   * @param type - 技能错误类型
   */
  private determineSkillErrorSeverity(type: SkillErrorType): ErrorSeverity {
  ...
  private determineSkillErrorStrategy(type: SkillErrorType): ErrorHandlingStrategy {
    const strategyMap: Record<SkillErrorType, ErrorHandlingStrategy> = {
   ...
   * @param type - 技能错误类型
   */
  private getSkillErrorRecoverySuggestion(type: SkillErrorType): string {
    const suggestionMap: Record<SkillErrorType, string> = {
   ...
   * @param error - 统一错误对象
   */
  private getErrorCategory(error: UnifiedError): string {
    switch (error.type) {
      case ErrorType.APP:
        return 'application';
      case ErrorType.SKILL:
  ...
  private getSeverityTitle(severity: ErrorSeverity): string {
    const titleMap: Record<ErrorSeverity, string> = {
   ...
   * @param severity - 严重级别
   */
  private getSeverityVariant(severity: ErrorSeverity): string {
    const variantMap: Record<ErrorSeverity, string> = {
   ...
   * @param error - 统一错误对象
   */
  private getModalActions(error: UnifiedError): Array<{label: string, action: string}> {
    const actions = [
      { label: '确定', action: 'close' }
    ];

    if (error.retryable) {
      actions.unshift({ label: '重试', action: 'retry' });
    }
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]): Promise<any> {
      try {
        ...
        const result = await unifiedErrorSystem.handleError(error, context, options);
        if (!result.recovered) {
          throw error;
        }
        return null;
