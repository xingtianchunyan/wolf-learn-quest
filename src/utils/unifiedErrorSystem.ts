...
private statistics: ErrorStatistics =  {
    ...
    byStrategy: {
} as Record<ErrorHandlingStrategy, number>,
    recovered: 0,
    failed: 0,
    retrySucceeded: 0,
    retryFailed: 0,
...
private initializeErrorHandlers(): void  { // 全局未捕获错误处理
  ...
  * @param options - 处理选项
  ...
  private async attemptRetry(
    error: UnifiedError,
    options: ErrorHandlingOptions
  ): Promise<boolean> { const retryKey = `${error.type 
}_${ error.code }`;
  ...
  * @param options - 处理选项
  ...
  private async executeFallback(
    ...
    try { switch (error.type) {
        ...
        default: return await this.genericFallback(error)
}
 ...
 * 生成错误ID
 */
private generateErrorId(): string { return `err_${Date.now() 
 ...
 * message函数的功能描述
 *
 * @param error.message - error.message参数
 * @returns void
 */
...
}> { const actions = [; { label: '确定', action: 'close'  
} ];
