 ...
 * delay函数的功能描述
 *
 * @param subscription.config.reconnectDelay - subscription.config.reconnectDelay参数
 * @returns void
 */
...
private estimateMessageSize(payload: any): number { try  {
    return JSON.stringify(payload).length * 2; // 粗略估算 } catch { return 1024; // 默认 1KB }
}
 ...
 * 生成频道名称
 */
private generateChannelName(config: SubscriptionConfig): string { return `enhanced_${config.table 
