...
* 订阅状态枚举
 */
export enum SubscriptionStatus { IDLE = 'idle',
...
* 内存优化级别枚举
 */
export enum MemoryOptimizationLevel { CONSERVATIVE = 'conservative', // 保守模式
...
* 订阅配置接口
 */
export interface SubscriptionConfig { table: string;
  filter?: string;
...
* 订阅信息接口
 */
export interface SubscriptionInfo { id: string;
  channelName: string;
...
* 内存统计接口
 */
export interface MemoryStats { totalSubscriptions: number;
...
* 增强的实时订阅管理器类（内存优化版）
 */
class EnhancedRealtimeManager { private static instance: EnhancedRealtimeManager;
  ...
  * 获取单例实例
   */
  public static getInstance(): EnhancedRealtimeManager { if (!EnhancedRealtimeManager.instance) {
  ...
  * 设置内存优化级别
   */
  public setMemoryOptimizationLevel(level: MemoryOptimizationLevel): void { this.memoryOptimizationLevel = level;
  ...
  * 创建订阅（内存优化版）
  ...
  public subscribe<T = any>(;
    ...
    // 检查是否已存在相同订阅
    if (this.subscriptions.has(subscriptionId)) { const existingCallbacks = this.callbacks.get(subscriptionId) || [];
    ...
    // 根据优先级决定是否立即连接
    if (subscriptionInfo.priority === 'high') { this.connect(subscriptionId);,
} else { // 延迟连接以减少并发
  ...
  * 取消订阅（内存优化版）
   */
  public unsubscribe(subscriptionId: string): void { const subscription = this.subscriptions.get(subscriptionId);
    ...
    try {
      ...
      // 清理弱引用
      if (subscription.weakRef) { subscription.weakRef = undefined;,
}
...
} catch (error) { logger.error('清理订阅失败', { subscriptionId, error  });,
}
  }
  ...
  * 连接订阅（内存优化版）
   */
  private async connect(subscriptionId: string): Promise<void> { const subscription = this.subscriptions.get(subscriptionId);
    ...
    try {
      ...
      channel.on(
        ...
        payload => { this.handleMessage(subscriptionId, payload);,
}
      );

      // 监听频道状态
      channel.on('system', {}, payload => { this.handleSystemMessage(subscriptionId, payload);,
});

      // 订阅频道
      await channel.subscribe((status, err) => { if (status === 'SUBSCRIBED') {
          ...
          // 启动心跳检测
          if (subscription.config.heartbeatInterval) { this.startHeartbeat(subscriptionId);,
}
        } else if (err) { subscription.status = SubscriptionStatus.ERROR;
          subscription.errorCount++;

          logger.error('订阅连接失败', { subscriptionId, error: err  });

          // 尝试重连
          if (subscription.config.enableReconnect) { this.scheduleReconnect(subscriptionId);,
}
        },
});

      subscription.channel = channel;,
} catch (error) { subscription.status = SubscriptionStatus.ERROR;
      subscription.errorCount++;
  ...
  * 处理消息（批处理优化版）
   */
  private handleMessage<T>(subscriptionId: string, payload: RealtimePostgresChangesPayload<T>): void { const subscription = this.subscriptions.get(subscriptionId);
    if (!subscription) return;
    ...
    // 批处理逻辑
    if (subscription.config.enableBatching) { this.handleBatchMessage(subscriptionId, payload);,
} else { // 直接处理消息
      const callbacks = this.callbacks.get(subscriptionId) || [];
      callbacks.forEach(callback => {
        try {
          callback(payload);,
} catch (error) { logger.error('回调执行失败', { subscriptionId, error  });,
}
      });,
}
  }
  ...
  * 处理批处理消息
   */
  private handleBatchMessage<T>(subscriptionId: string, payload: RealtimePostgresChangesPayload<T>): void { const subscription = this.subscriptions.get(subscriptionId);
    if (!subscription) return;
  ...
  * 刷新批处理消息
   */
  private flushBatchMessages(subscriptionId: string): void { const subscription = this.subscriptions.get(subscriptionId);
    ...
    if (batchCallbacks.length > 0) { batchCallbacks.forEach(callback => {
        try {
          callback(messages);,
} catch (error) { logger.error('批处理回调执行失败', { subscriptionId, error  });,
}
      });,
} else { // 如果没有批处理回调，逐个处理消息
      const callbacks = this.callbacks.get(subscriptionId) || [];
  ...
  * 处理系统消息
   */
  private handleSystemMessage(subscriptionId: string, payload: any): void { const subscription = this.subscriptions.get(subscriptionId);
    if (!subscription) return;
  ...
  * 启动心跳检测
   */
  private startHeartbeat(subscriptionId: string): void { const subscription = this.subscriptions.get(subscriptionId);
  ...
  * 安排重连
   */
  private scheduleReconnect(subscriptionId: string): void { const subscription = this.subscriptions.get(subscriptionId);
    if (!subscription) return;
    ...
    const timer = setTimeout(() => { this.connect(subscriptionId);,
}, delay);
  ...
  * 清理定时器
   */
  private clearTimers(subscriptionId: string): void { const heartbeatTimer = this.heartbeatTimers.get(subscriptionId);
    if (heartbeatTimer) {
  ...
  * 启动内存管理
   */
  private startMemoryManagement(): void { // 定期清理
    this.cleanupTimer = setInterval(() => {
      this.performRoutineCleanup();,
}, this.CLEANUP_INTERVAL);

    // 内存检查
    this.memoryCheckTimer = setInterval(() => { this.checkMemoryUsage();,
}, this.MEMORY_CHECK_INTERVAL);

    // 暂停检查
    this.suspensionTimer = setInterval(() => { this.checkSuspendedSubscriptions();,
  ...
  * 执行常规清理
   */
  private performRoutineCleanup(): void { const now = Date.now();
    ...
    this.subscriptions.forEach((subscription, id) => {
      ...
      // 清理错误过多的订阅
      if (subscription.errorCount > 5) { subscriptionsToClean.push(id);,
}

      // 清理内存使用过高的订阅
      if (subscription.memoryUsage > this.MAX_MEMORY_PER_SUBSCRIPTION * 1.5) { subscriptionsToClean.push(id);,
}
    });

    subscriptionsToClean.forEach(id => { logger.info('清理空闲/错误订阅', { subscriptionId: id  });
      this.unsubscribe(id);,
});
  ...
  * 检查内存使用
   */
  private checkMemoryUsage(): void { const stats = this.getMemoryStats();
    ...
    // 根据内存使用情况调整优化级别
    if (stats.totalMemoryUsage > this.MAX_TOTAL_MEMORY * 0.8) { this.setMemoryOptimizationLevel(MemoryOptimizationLevel.AGGRESSIVE);,
} else if (stats.totalMemoryUsage < this.MAX_TOTAL_MEMORY * 0.5) { this.setMemoryOptimizationLevel(MemoryOptimizationLevel.CONSERVATIVE);,
}
  }
  ...
  * 检查暂停的订阅
   */
  private checkSuspendedSubscriptions(): void { this.subscriptions.forEach((subscription, id) => {
...
* 执行紧急清理
 */
private performEmergencyCleanup(): void { logger.warn('执行紧急清理');
  ...
  for (let i = 0; i < toClean; i++) { const [id, subscription] = subscriptionsByPriority[i];
...
* 优化订阅内存
 */
private optimizeSubscriptionMemory(subscriptionId: string): void { const subscription = this.subscriptions.get(subscriptionId);
  if (!subscription) return;
...
* 估算消息大小
 */
private estimateMessageSize(payload: any): number { try {
    return JSON.stringify(payload).length * 2; // 粗略估算,
} catch { return 1024; // 默认 1KB,
}
}
...
* 检查是否可以创建订阅
 */
private canCreateSubscription(): boolean { const stats = this.getMemoryStats();
  return (;
...
* 调整优化参数
 */
private adjustOptimizationParameters(): void { switch (this.memoryOptimizationLevel) {
...
* 获取内存统计
 */
public getMemoryStats(): MemoryStats { let totalMemoryUsage = 0;
  ...
  this.subscriptions.forEach(subscription => {
    ...
    if (now - subscription.lastActivity > subscription.config.maxIdleTime!) { idleCount++;,
}

    if (subscription.status === SubscriptionStatus.SUSPENDED) { suspendedCount++;,
}
  });

  return { totalSubscriptions: this.subscriptions.size,
    totalMemoryUsage,
...
* 获取统计信息
 */
public getStats() { const subscriptions: Record<string, any> = { };

  this.subscriptions.forEach((subscription, id) => { subscriptions[id] = {
...
public destroy(): void { this.isDestroyed = true;
  ...
  if (this.suspensionTimer) { clearInterval(this.suspensionTimer);
...
* 生成订阅ID
 */
private generateSubscriptionId(config: SubscriptionConfig): string { const parts = [;
    config.table,
...
* 生成频道名称
 */
private generateChannelName(config: SubscriptionConfig): string { return `enhanced_${config.table }_${ Date.now() }_${ Math.random().toString(36).substr(2, 9) }`;,
}
}
...
export function useEnhancedRealtime<T = any>(;
  ...
  const memoizedCallback = useCallback((payload: RealtimePostgresChangesPayload<T>) => { callbackRef.current(payload);,
}, []);
