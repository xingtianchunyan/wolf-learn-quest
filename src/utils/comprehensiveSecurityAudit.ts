...
* 安全审计结果接口
 */
export interface SecurityAuditResult { auditId: string;
  timestamp: string;
  overallScore: number;
...
} catch (error) { logger.error(`自定义规则执行失败: ${customRule.name }`, error);,
}
      }
...
private calculateOverallScore(): number { if (this.vulnerabilities.length === 0) return 100;
  ...
  const totalDeduction = this.vulnerabilities.reduce((sum, vuln) => { return sum + weights[vuln.severity];,
}, 0);
...
private calculateCategoryScore(vulnerabilities: SecurityVulnerability[]): number { if (vulnerabilities.length === 0) return 100;
  ...
  const totalDeduction = vulnerabilities.reduce((sum, vuln) => { return sum + weights[vuln.severity];,
}, 0);
...
private generateRecommendations() { const immediate: string[] = [];
  ...
  // 短期目标（高危漏洞）
  highVulns.forEach(vuln => { shortTerm.push(vuln.recommendation);,
});

  // 长期目标（中危漏洞）
  mediumVulns.forEach(vuln => { longTerm.push(vuln.recommendation);,
});
...
* 生成漏洞ID
 */
private generateVulnerabilityId(): string { return `vuln_${Date.now() }_${ Math.random().toString(36).substr(2, 9) }`;,
}
