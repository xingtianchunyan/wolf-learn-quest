...
export class ComprehensiveSecurityAudit {
  private static instance: ComprehensiveSecurityAudit;
   ...
   * 获取单例实例
   */
  public static getInstance(): ComprehensiveSecurityAudit {
    if (!ComprehensiveSecurityAudit.instance) {
  ...
  public async performComprehensiveAudit(
    ...
    try {
      ...
      // 6. XSS防护审计
      if (!auditConfig.excludeCategories.includes(VulnerabilityType.XSS)) {
        await this.auditXSSProtection();
      }

      // 7. CSRF防护审计
      if (!auditConfig.excludeCategories.includes(VulnerabilityType.CSRF)) {
  ...
  private async auditInputValidation(): Promise<void> {
    ...
    if (allowedDangerousTypes && allowedDangerousTypes.length > 0) {
      this.addVulnerability({
        type: VulnerabilityType.INPUT_VALIDATION,
  ...
  private async auditAuthentication(): Promise<void> {
    ...
    // 检查登录尝试限制
    if (globalConfig.maxLoginAttempts > 10) {
      this.addVulnerability({
        type: VulnerabilityType.AUTHENTICATION,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '登录尝试限制过宽松',
    ...
    // 检查双因素认证
    if (!globalConfig.enableTwoFactor) {
      this.addVulnerability({
        type: VulnerabilityType.AUTHENTICATION,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '未启用双因素认证',
  ...
  private async auditSessionManagement(): Promise<void> {
    ...
    // 检查会话固定攻击防护
    // 检查登录后是否重新生成会话ID
    if (!sessionConfig.regenerateSessionOnLogin) {
      this.addVulnerability({
        type: VulnerabilityType.SESSION_MANAGEMENT,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '缺少会话重新生成机制',
   ...
   * 审计注入攻击防护
   */
  private async auditInjectionProtection(): Promise<void> {
    logger.debug('开始注入攻击防护审计');
  ...
  private async auditErrorHandling(): Promise<void> {
    ...
    // 检查错误日志记录
    // 检查是否正确记录安全相关错误
    if (!errorConfig.enableSecurityLogging) {
      this.addVulnerability({
        type: VulnerabilityType.ERROR_HANDLING,
        severity: VulnerabilitySeverity.LOW,
        title: '安全日志记录未启用',
  ...
  private async auditDataExposure(): Promise<void> {
    ...
    // 检查日志中是否包含敏感信息
    if (!dataProtectionConfig.sanitizeLogs) {
      this.addVulnerability({
        type: VulnerabilityType.DATA_EXPOSURE,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '日志清理未启用',
    ...
    // 检查API响应是否暴露敏感数据
    if (!dataProtectionConfig.filterSensitiveFields) {
      this.addVulnerability({
        type: VulnerabilityType.DATA_EXPOSURE,
        severity: VulnerabilitySeverity.MEDIUM,
        title: 'API响应未过滤敏感字段',
    ...
    // 检查数据备份安全
    if (!dataProtectionConfig.secureBackups) {
      this.addVulnerability({
        type: VulnerabilityType.DATA_EXPOSURE,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '数据备份安全措施不足',
  ...
  private async auditBusinessLogic(): Promise<void> {
    ...
    // 检查竞态条件
    if (!businessConfig.enableConcurrencyControl) {
      this.addVulnerability({
        type: VulnerabilityType.BUSINESS_LOGIC,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '缺少并发控制机制',
    ...
    // 检查业务流程完整性
    if (!businessConfig.validateWorkflow) {
      this.addVulnerability({
        type: VulnerabilityType.BUSINESS_LOGIC,
        severity: VulnerabilitySeverity.LOW,
        title: '业务流程验证不完整',
    ...
    // 检查数据一致性验证
    if (!businessConfig.enforceDataConsistency) {
      this.addVulnerability({
        type: VulnerabilityType.BUSINESS_LOGIC,
        severity: VulnerabilitySeverity.MEDIUM,
        title: '数据一致性验证不足',
    ...
    // 检查业务限制执行
    if (!businessConfig.enforceBusinessLimits) {
      this.addVulnerability({
        type: VulnerabilityType.BUSINESS_LOGIC,
        severity: VulnerabilitySeverity.LOW,
        title: '业务限制执行不严格',
  ...
  private categorizeVulnerabilities() {
    const categories = {} as SecurityAuditResult['categories'];
  ...
  private generateRecommendations() {
    ...
    if (this.vulnerabilities.length > 10) {
      immediate.push('建立定期安全审计流程');
    }

    if (criticalVulns.length > 0) {
      immediate.push('立即修复所有关键安全漏洞');
    }
   ...
   * 生成HTML报告
   */
  private generateHTMLReport(result: SecurityAuditResult): string {
    return `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body>
    <div class="header">
        <h1>安全审计报告</h1>
        <p><strong>审计ID:</strong> ${result.auditId}</p>
        <p><strong>生成时间:</strong> ${new Date(result.timestamp).toLocaleString()}</p>
        <p><strong>总体评分:</strong> ${result.overallScore}/100</p>
        <p><strong>风险级别:</strong> ${result.riskLevel}</p>
    </div>

    <div class="summary">
        <div class="metric">
            <h3>漏洞统计</h3>
            <p>总计: ${result.summary.totalVulnerabilities}</p>
            <p>关键: ${result.summary.criticalCount}</p>
            <p>高危: ${result.summary.highCount}</p>
            <p>中危: ${result.summary.mediumCount}</p>
            <p>低危: ${result.summary.lowCount}</p>
        </div>
        
        <div class="metric">
            <h3>合规性状态</h3>
            <p>OWASP: ${result.complianceStatus.owasp.score}/100</p>
            <p>GDPR: ${result.complianceStatus.gdpr.compliant ? '合规' : '不合规'}</p>
            <p>ISO 27001: ${result.complianceStatus.iso27001.score}/100</p>
        </div>
    </div>
