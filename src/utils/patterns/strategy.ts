...
export class StrategyContext<TInput = any, TOutput = any> { private strategy: Strategy<TInput, TOutput> | null = null;
  ...
  * @param input - 输入参数
  ...
  async executeBatch(
    ...
    input: TInput
  ): Promise<Array<{ name: string; result: TOutput; error?: Error  }>> { logger.debug('策略上下文批量执行策略', {
...
export function useStrategyContext<TInput = any, TOutput = any>(;
  ...
  const setStrategy = React.useCallback((name: string) => { context.setStrategy(name);,
}, [context]);

  const execute = React.useCallback((input: TInput) => { return context.execute(input);,
}, [context]);

  const executeStrategy = React.useCallback((name: string, input: TInput) => { return context.executeStrategy(name, input);,
}, [context]);
  ...
  const getStrategyNames = React.useCallback(() => { return context.getStrategyNames();,
}, [context]);
...
export function useConditionalStrategy<TInput = any, TOutput = any>(): { selector: ConditionalStrategySelector<TInput, TOutput>;
  ...
  const execute = React.useCallback((input: TInput) => { return selector.execute(input);,
}, [selector]);

  const selectStrategy = React.useCallback((input: TInput) => { return selector.selectStrategy(input);,
}, [selector]);
