...
setMaxListeners(max: number): void  {
  ...
  * @returns 订阅ID
   */
private generateSubscriptionId(): string { return `sub_${++this.subscriptionCounter 
}_$ { Date.now() }`
}
...
export function useEventEmitter():  { emit: <T = any>(eventName: string, data?: T) => Promise<boolean>;
  ...
  const emitSync = React.useCallback(;
    <T = any>(eventName: string, data?: T) => {
  return globalEventEmitter.emitSync(eventName, data)
