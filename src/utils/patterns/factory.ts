...
export abstract class AbstractFactory<T> implements Factory<T> { protected registry: Map<string, new (...args: any[]) => T> = new Map();
  ...
  * @param configs - 产品配置数组
   */
  createBatch(configs: Array<{ type: string; args?: any[]  }>): T[] { logger.debug('工厂批量创建产品', { count: configs.length  });

    return configs.map(config => { return this.create(config.type, ...(config.args || []));,
});,
}
...
export class AsyncFactory<T> { private registry: Map<string, (...args: any[]) => Promise<T>> = new Map();
  ...
  * @param configs - 产品配置数组
   */
  async createBatch(configs: Array<{ type: string; args?: any[]  }>): Promise<T[]> { logger.debug('异步工厂批量创建产品', { count: configs.length  });

    const promises = configs.map(config => { return this.create(config.type, ...(config.args || []));,
});
...
export function useAsyncFactory<T>(factory: AsyncFactory<T>): { factory: AsyncFactory<T>;
  ...
  createBatch: (configs: Array<{ type: string; args?: any[]  }>) => Promise<T[]>;,
