...
export interface SkillEffectConfig { type: string;
  priority: number;
  duration?: number;
  conditions?: any;
  data: any;,
...
export const SKILL_EFFECT_CONFIGS = { // 狼人击杀
  ...
  // 女巫解药
  witch_antidote: { type: 'protection',
    priority: 95,
    duration: 86400, // 24小时
    data: {
      effect_type: 'heal',
  ...
  // 预言家查验
  seer_investigation: { type: 'investigation',
    priority: 70,
    data: {
  ...
  // 猎人反击
  hunter_revenge: { type: 'elimination',
    priority: 100, // 最高优先级
    data: {
...
export const resolveSkillEffects = (;
  ...
  resolution.forEach((conflict: any) => { if (conflict.resolution === 'protection_wins') {
      // 移除被保护的伤害效果
...
export const calculateEffectDuration = (;
  ...
  // 根据游戏阶段计算默认持续时间
  switch (effect.type) { case 'protection':
