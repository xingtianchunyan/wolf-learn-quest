...
// 标准化的技能使用记录
export interface StandardizedSkillUse {
  id: string;
  user_id: string;
...
// 标准化的技能效果队列
export interface StandardizedSkillEffectsQueue {
  id: string;
  skill_use_id: string;
  game_state_id: string;
  room_id: string;
...
// 标准化的技能目标记录
export interface StandardizedSkillTarget {
  id: string;
  skill_use_id: string;
...
export function standardizeSkillUse(
  ...
  if (!rawSkillUse?.id || !rawSkillUse?.skill_name) {
...
export function standardizeSkillEffectsQueue(
  rawQueue: any
): StandardizedSkillEffectsQueue | null {
  if (!rawQueue?.id || !rawQueue?.effect_type) {
  ...
  for (const rawUse of skillUses) {
    try {
      ...
      if (standardized) {
        ...
        errors.push(`无法标准化技能使用记录: ${rawUse.id}`);
      }
    } catch (error: any) {
      errors.push(`标准化失败 (${rawUse.id}): ${error.message}`);
    }
  }
...
export function validateDataConsistency(
  ...
  for (const use of skillUses) {
    ...
    // 数据类型检查
    if (use.round_number && typeof use.round_number !== 'number') {
      warnings.push(`技能使用记录 ${use.id} 的 round_number 类型不正确`);
      inconsistentFieldCount++;
    }

    // 技能效果格式检查
    if (use.skill_effects && typeof use.skill_effects === 'object') {
      const standardized = standardizeEffectData(use.skill_effects);
      if (!standardized) {
  ...
  for (const target of skillTargets) {
    ...
    // 检查效果数据格式
    if (target.effect_applied) {
      const standardized = standardizeEffectData(target.effect_applied);
      if (!standardized) {
  ...
  // 验证技能效果队列
  for (const queue of skillEffectsQueue) {
    // 检查关联的技能使用记录是否存在
    ...
    // 检查效果数据格式
    if (queue.effect_data) {
      const standardized = standardizeEffectData(queue.effect_data);
      if (!standardized) {
