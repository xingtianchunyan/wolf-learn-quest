...
export class GlobalErrorMonitor {
   ...
   */
  public static getInstance(): GlobalErrorMonitor {
   ...
   * 处理捕获到的全局错误
   */
  private async handleGlobalError(error: Error, context: any) {
    if (!this.shouldReportError(error, context)) return;
   ...
   * 根据配置过滤规则判断是否需要报告错误
   */
  private shouldReportError(error: Error, context: any): boolean {
    // 检查采样率
    ...
    if (this.config.filters.ignoreUrls.some(pattern => url.includes(pattern))) {
      return false;
    }
   ...
   * 更新错误统计数据
   */
  private updateStats(error: Error, context: any) {
    ...
    // 限制时间序列长度
    if (this.stats.errorsByTime.length > 1000) {
      this.stats.errorsByTime.shift();
    }
  ...
  private updateTopErrors(message: string) {
    ...
    if (this.stats.topErrors.length > 10) {
  ...
  private updateErrorRateTrend(timestamp: number) {
    ...
    // 限制趋势数据长度
    if (this.stats.errorRateTrend.length > 100) {
      this.stats.errorRateTrend.shift();
    }
  }
   ...
   * 评估错误对应用性能的影响程度
   */
  private calculateErrorImpactOnPerformance(): number {
    const recentErrors = this.stats.errorsByTime.filter(
...
export function getGlobalErrorMonitor() {
  return GlobalErrorMonitor.getInstance();
}
