...
export class DocumentationGenerator {
  ...
  private async getUtilFiles(): Promise<string[]> {
    const allFiles = await this.getAllFiles(this.config.srcDir);
    return allFiles.filter(
      file => file.endsWith('.ts') && file.includes('/utils/')
    );
  }
  ...
  getStatistics() {
    return {
      ...
      documentedComponents: this.components.filter(c => c.hasDocumentation)
        .length,
