...
export class ErrorClassifier implements IErrorClassifier {
   ...
   * 分类错误
   */
  public classify(error: any): ErrorClassification {
    // 查找匹配的错误模式
   ...
   * 批量分类错误
   */
  public classifyBatch(errors: any[]): ErrorClassification[] {
  ...
  public analyzeTrends(
    ...
    const recentErrors = errors.filter(
      error => (error.timestamp || Date.now()) > now - timeWindow
    );
    ...
    return {
      ...
      recoverySuccess:
        recentErrors.length > 0
          ? successfulRecoveries / recentErrors.length
          : 0,
    };
  }
  ...
  private initializeErrorPatterns(): void {
    this.errorPatterns = [
      // 网络错误模式
      {
        ...
        userDescriptionGenerator: () =>
          '网络连接出现问题，请检查网络设置后重试',
        technicalDescriptionGenerator: error =>
      ...
      // 验证错误模式
      {
        ...
        userDescriptionGenerator: error =>
          `输入数据有误: ${this.extractValidationMessage(error)}`,
        technicalDescriptionGenerator: error =>
   ...
   * 检查是否为技能错误
   */
  private isSkillError(error: any): boolean {
    if (!error) return false;
  ...
  private extractValidationMessage(error: any): string {
    if (error.details?.field && error.details?.message) {
   ...
   * 获取技能错误消息
   */
  private getSkillErrorMessage(error: any): string {
    const skillErrorMessages = {
   ...
   * 获取默认分类
   */
  private getDefaultClassification(error: any): ErrorClassification {
    return {
      type: 'unknown',
      category: 'general',
      severity: ErrorSeverity.MEDIUM,
