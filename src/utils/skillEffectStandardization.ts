...
export interface StandardizedEffectData { effect_type: string;
  target_type?: string;
  duration?: number;
  stack_count?: number;
 ...
 */
export function standardizeEffectData(effectApplied: any): StandardizedEffectData | null { if (!effectApplied || typeof effectApplied !== 'object') {
    return null;,
}
...
* 兼容新旧字段名
 */
export function matchesEffectType(effectApplied: any, targetEffectType: string): boolean { if (!effectApplied || !targetEffectType) {
    return false;,
}
...
* 批量标准化技能目标数据
 */
export function standardizeSkillTargets(targets: any[]): any[] { return targets.map(target => ({
    ...target,
...
* 这是一个一次性数据迁移的辅助函数
 */
export function migrateEffectAppliedData(oldEffectApplied: any): StandardizedEffectData | null { if (!oldEffectApplied) return null;
  ...
  // 如果只有 type 字段，进行迁移
  if (oldEffectApplied.type) { return {
