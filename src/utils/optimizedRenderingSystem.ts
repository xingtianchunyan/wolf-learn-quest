 ...
 * 优化的渲染系统类
 */
class OptimizedRenderingSystem {
  private static instance: OptimizedRenderingSystem;
   ...
   * 获取单例实例
   */
  public static getInstance(): OptimizedRenderingSystem {
    if (!OptimizedRenderingSystem.instance) {
  ...
  private recordRenderMetric(componentName: string, renderTime: number): void {
    const existing = this.renderingMetrics.get(componentName);
   ...
   * 检查性能警告
   */
  private checkPerformanceWarnings(metrics: RenderingMetrics): void {
    // 渲染时间过长警告
   ...
   * 获取渲染指标
   */
  public getRenderingMetrics(componentName?: string): RenderingMetrics | Map<string, RenderingMetrics> {
    if (componentName) {
   ...
   * 清除渲染指标
   */
  public clearRenderingMetrics(componentName?: string): void {
    if (componentName) {
   ...
   * 更新配置
   */
  public updateConfig(newConfig: Partial<RenderingConfig>): void {
 ...
 * 优化的渲染 Hook
 */
export function useOptimizedRendering(componentName: string, dependencies: any[] = []) {
  ...
  const endRenderTiming = useCallback(() => {
    if (optimizedRenderingSystem.getConfig().enableProfiling && renderStartTimeRef.current > 0) {
...
export function useOptimizedMemo<T>(
  ...
  return useMemo(() => {
    ...
    if (componentName && optimizedRenderingSystem.getConfig().enableProfiling && memoStartTime.current > 0) {
...
export function useOptimizedCallback<T extends (...args: any[]) => any>(
  ...
  return useCallback((...args: Parameters<T>) => {
    ...
    if (componentName && optimizedRenderingSystem.getConfig().enableProfiling && callbackStartTime.current > 0) {
...
export function usePerformanceMonitor(componentName: string) {
  const metrics = optimizedRenderingSystem.getRenderingMetrics(componentName) as RenderingMetrics;
