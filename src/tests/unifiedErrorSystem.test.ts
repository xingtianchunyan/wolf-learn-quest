...
describe('统一错误处理系统测试', () => {
  ...
  describe('重试机制测试', () => {
    it('应该正确执行重试逻辑', async () => {
      ...
      const result = await errorSystem.handleError(retryableError, {}, {
        attemptRecovery: true,
        customRecovery,
        maxRetries: 3,
        retryDelay: 100
  ...
  describe('错误统计测试', () => {
    ...
    it('应该正确更新统计信息', async () => {
      ...
      await unifiedErrorSystem.handleError(error1, { component: 'TestComponent1' });
      await unifiedErrorSystem.handleError(error2, { component: 'TestComponent2' });
