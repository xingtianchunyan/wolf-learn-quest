...
private async simulateSystemStartup(): Promise<void>  {
  ...
  async createGameSession(
    ...
    const gameState: GameState = { gameId,
      phase: 'lobby',
      players,
      round: 0,
      status: 'waiting'  
...
async simulateConcurrentUsers(userCount: number): Promise<SystemMetrics>  {
    ...
    const metrics: SystemMetrics = { cpuUsage: Math.min(90, 10 + userCount * 0.5), // 降低CPU使用率增长
...
private async simulateRecovery(): Promise<void>  {
   ...
   * 更新系统性能指标
   */
private updateSystemMetrics(): void  { const lastMetrics = this.systemMetrics[this.systemMetrics.length - 1];
