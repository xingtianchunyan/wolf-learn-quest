...
export const useAutoDyingStatusProcessor = (
  ...
  // 清理定时器
  const clearPlayerTimeout = (userId: string) => {
  ...
  const checkProtectionEffects = async (
    ...
    try {
      ...
      if (error) {
        console.error('查询保护效果失败:', error);
        return false;
      }

      return protectionEffects && protectionEffects.length > 0;
    } catch (error) {
      console.error('检查保护效果异常:', error);
      return false;
    }
  };
  ...
  const checkPlayerAnswerResult = async (
    ...
    try {
      ...
      if (gameError || !gameData) {
        console.error('获取游戏状态失败:', gameError);
        return null;
      }
      ...
      if (answersError) {
        console.error('查询答题记录失败:', answersError);
        return null;
      }
      ...
      console.error('检查答题结果异常:', error);
      return null;
    }
  };

  // 处理新的濒死状态
  const handleNewDyingStatus = (userId: string, gameStateId: string) => {
    // 清除该玩家之前的定时器
    ...
    // 设置答题结果判断定时器
    if (effectiveConfig.enableAnswerBasedResolution) {
  ...
  return {
    isProcessing: (userId: string) => processingRef.current.has(userId),
    clearPlayerTimeout,
    clearAllTimeouts,
  };
};
