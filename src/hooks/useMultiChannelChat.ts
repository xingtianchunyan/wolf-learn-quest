...
interface UseMultiChannelChatProps { roomId: string | null;
  currentUser: any;
  userRole?: string
}
 ...
 * 接口注释：系统公告可见性结构
 */
interface AnnouncementVisibility  { isVisibleToJudge: boolean;
 ...
 */
interface SystemAnnouncementMetadata  { visibility?: AnnouncementVisibility;
  data?: {
    actorUserId?: string;
    targetUserId?: string;
    roomId?: string
...
/**
 * useMultiChannelChat函数
 * 自定义Hook
 * @returns void
 */
export const useMultiChannelChat = ( { roomId,
  currentUser,
  userRole }: UseMultiChannelChatProps) => { const [messages, setMessages] = useState<ChatMessage[]>([]);
  ...
  * - 使用消息 metadata.visibility 进行细粒度判定
   */
const canViewSystemAnnouncement = useCallback((metadata: SystemAnnouncementMetadata | undefined): boolean =>  { if (!metadata || !metadata.visibility) return true; // 无可见性配置则默认可见（与现有行为保持兼容）
  ...
  // 确定可用频道
/**
 * getAvailableChannels函数
 * 获取数据
 * @returns void
 */
const getAvailableChannels = (): ChatChannel[] =>  { const baseChannels: ChatChannel[] = ['public', 'judge_private', 'system', 'all'];
...
const fetchMessages = async () =>  {
      try {
        ...
        const messagesWithSenders = await Promise.all(;
          (data || []).map(async msg => { const { data: userData  
} = await supabase;
 ...
 * sendMessage函数的功能描述
 *
 * @param messageText - messageText参数
 * @param chatType - chatType参数
 * @returns Promise<void>
 */
const sendMessage = async (messageText: string, chatType: string = 'public') =>  { // 映射UI频道类型到数据库类型
...
/**
 * getFilteredMessages函数
 * 获取数据
 * @returns void
 */
const getFilteredMessages = () =>  { let filteredMessages = currentChannel === 'all';
    ? messages
    : messages.filter(msg => {
    ...
    // 对系统公告应用可见性规则
    if (currentChannel === 'system' || currentChannel === 'all') { filteredMessages = filteredMessages.filter(msg => {
 ...
 */
  ...
  return { messages: getFilteredMessages(),
    isLoading,
    sendMessage,
    currentChannel,
    setCurrentChannel,
