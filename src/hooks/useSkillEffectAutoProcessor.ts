...
export interface ProcessorStats { totalProcessed: number;
  successCount: number;
  failureCount: number;
...
export const useSkillEffectAutoProcessor = (
  ...
  // 核心处理函数 - 修复技能效果处理流程
  const processSkillEffects = useCallback(async (): Promise<number> => { if (!gameStateId || processingRef.current) {
      return 0
}
    ...
    try { logger.info('开始处理技能效果', { gameStateId  });
      ...
      // 更新统计数据
      setStats(prev => ({ totalProcessed: prev.totalProcessed + processedCount,
      ...
      if (processedCount > 0) { logger.info(`成功处理 ${processedCount } 个技能效果`, { gameStateId,
          processingTime,
          processedCount });
        ...
        if (isJudge) { toast({
            title: '技能效果处理完成',
            description: `成功处理了 ${processedCount 
} 个技能效果` })
}
      }
  ...
  // 启动自动处理
  const startAutoProcess = useCallback(() => { if (!gameStateId || !isJudge || stats.isRunning) {
      return
}
    ...
    } catch (error) { // 错误已在processSkillEffects中处理 }
  }, 10000); // 10秒间隔
...
// 阶段切换时自动处理技能效果
useEffect(() => { if (gameStateId && isJudge && currentPhase) {
    // 在阶段切换后延迟2秒处理技能效果
...
// 自动启动和清理
useEffect(() => { if (gameStateId && isJudge) {
    startAutoProcess()
}
