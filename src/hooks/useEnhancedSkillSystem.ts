...
export const useEnhancedSkillSystem = (
  ...
  // 内存管理
  const memoryManager = useMemoryManager({ componentName: 'EnhancedSkillSystem',
    maxMemoryThreshold: 30, // 30MB
    enableAutoCleanup: true 
});

  // 智能缓存 - 技能配置数据
  const { data: skillConfigs  
} = useIntelligentCache(;
  ...
  const skillUsesSubscription = useEnhancedRealtime(;
    { table: 'skill_uses',
      filter: gameStateId ? `game_state_id=eq.${gameStateId 
}` : undefined,
      event: '*',
...
} else if (newPayload.eventType === 'UPDATE') { skillDataHook.setSkillUses(current =>;
            current.map(su => su.id === newSkillUse.id ? newSkillUse : su);
          )
} else if (newPayload.eventType === 'DELETE' && newPayload.old) { skillDataHook.setSkillUses(current =>;
...
// 注册订阅清理
React.useEffect(() => { performanceFixes.subscriptionManager.add(() => {
  // 订阅会自动清理，这里只是注册到管理器 })

}, [performanceFixes.subscriptionManager]);

// 智能缓存管理 - 使用缓存管理器
React.useEffect(() => { const cacheMaintenanceInterval = setInterval(() => {
...
// 预加载关键数据
React.useEffect(() => { if (gameStateId) {
...
return { // 数据状态
  ...
  // 新增性能修复接口
  subscriptionStatus: { skillUses: skillUsesSubscription.status,
