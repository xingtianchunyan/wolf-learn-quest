...
export interface PotionUsageStatus { protectionUsed: boolean;
  attackUsed: boolean;
...
export const useWitchPotionManager = (
  ...
  // 检查魔药使用状态
  const checkPotionStatus = useCallback(async () => { if (!gameStateId || !userId) return;
  ...
  // 使用保护魔药
  const useProtectionPotion = useCallback(async (targetUserId?: string) => { if (!potionStatus.canUseProtection) {
      toast({
        title: '无法使用保护魔药',
    ...
    setLoading(true);
    try { const { data, error  } = await supabase.rpc('use_skill_enhanced', { p_game_state_id: gameStateId,
  ...
  // 使用攻击魔药
  const useAttackPotion = useCallback(async (targetUserId: string) => { if (!potionStatus.canUseAttack) {
      toast({
        title: '无法使用攻击魔药',
