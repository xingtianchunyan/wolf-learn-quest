...
export const useWitchPotionManager = (
  ...
  const checkPotionStatus = useCallback(async () => {
    ...
    try {
      ...
      // 验证保护魔药可用性
      const protectionValidation = await supabase.rpc('validate_witch_potion_usage', {
        p_user_id: userId,
      ...
      // 验证攻击魔药可用性
      const attackValidation = await supabase.rpc('validate_witch_potion_usage', {
        p_user_id: userId,
        p_game_state_id: gameStateId,
  ...
  // 使用保护魔药
  const useProtectionPotion = useCallback(async (targetUserId?: string) => {
  ...
  // 使用攻击魔药
  const useAttackPotion = useCallback(async (targetUserId: string) => {
