...
import { useGameState  } from
  type GameState,
  type GameSettings  } from '../useGameState';
...
describe('useGameState', () => { const mockRoomId = 'test-room-id';
  ...
  const mockGameSettings: GameSettings = { id: 'test-settings-id',
    roomId: 'test-room-id',
    isAutoAdvance: true,
    dayDuration: 300,
    eveningDuration: 180,
...
it('应该正确处理游戏状态的实时更新', async () =>  {
  ...
  expect(result.current.loading).toBe(false)
      ...
      // 模拟实时更新
      const updatedGameState = { ...mockGameState,
...
it('应该正确处理游戏设置的实时更新', async () =>  {
  ...
  expect(result.current.loading).toBe(false)
      ...
      // 模拟实时更新
      const updatedGameSettings = { ...mockGameSettings,
...
it('应该正确计算阶段剩余时间', async () =>  {
      ...
      const gameStateWithEndTime = { ...mockGameState,
        phaseEndTime: '2024-01-01T01:00:00Z',
        isPaused: false  
};

      vi.mocked(supabase.from).mockImplementation((table: string) => { const mockQuery = {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockReturnThis(),
          maybeSingle: vi.fn()  
};
 ...
 * 测试游戏暂停时的时间计算
 */
it('应该在游戏暂停时将剩余时间设为0', async () =>  { // Mock paused game state
 ...
 * 测试阶段结束时间为空的情况
 */
it('应该在阶段结束时间为空时将剩余时间设为0', async () =>  { // Mock game state without end time
