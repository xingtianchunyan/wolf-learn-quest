...
import {
  enhancedUserNotificationSystem, ErrorSeverity   } from '@/utils/unifiedErrorHandler';
import {
  unifiedErrorManager, useCallback, useEffect, useRef, useState   } from 'react';
 ...
 */
  ...
  ErrorHandlingResult  } from '@/utils/unifiedErrorManager';
  NotificationType,
  NotificationPosition  } from '@/utils/enhancedUserNotificationSystem';
...
* Hook选项接口
 */
export interface UseUnifiedErrorHandlingOptions  { /** 组件名称（用于错误上下文） */
...
* Hook返回值接口
 */
export interface UseUnifiedErrorHandlingReturn  { /** 处理错误 */
...
export function useUnifiedErrorHandling(
...
): UseUnifiedErrorHandlingReturn { const {
    componentName,
    defaultContext = { },
    defaultOptions = {},
    autoNotify = true,
  ...
  // 更新错误统计
  const updateErrorStats = useCallback(() => { const stats = unifiedErrorManager.getErrorStats();
  ...
  const handleError = useCallback(async (;
    ...
    options: Partial<ErrorHandlingOptions> = {
  ...
  ): Promise<ErrorHandlingResult> => { try {
      // 构建完整的错误上下文
      ...
      // 构建完整的处理选项
      const fullOptions: ErrorHandlingOptions = { showNotification: autoNotify,
        recoveryStrategy,
        autoRetry: true,
        retryDelay,
        ...defaultOptions,
      ...
      logger.debug('错误处理完成', { component: componentRef.current,
        errorId: result.errorId,
        handled: result.handled 
});

      return result
} catch (handlingError) { logger.error('错误处理失败', handlingError);
...
} catch (error) { await handleError(error, context);
        return null
}
    }
}, [handleError]);
  ...
  const retry = useCallback(async <T>(;
  ...
  ): Promise<T | null> => { const maxAttempts = retryOptions.maxRetries || maxRetries;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        // 记录重试失败
        logger.warn('重试失败，继续尝试', { component: componentRef.current,
          attempt,
          maxAttempts,
  ...
  // 通知方法
  const notify = { success: useCallback((message: string) => {
      enhancedUserNotificationSystem.success(message, {
        position: notificationPosition 
})
}, [notificationPosition]),

    error: useCallback((message: string) => { enhancedUserNotificationSystem.error(message, {
        position: notificationPosition 
})
}, [notificationPosition]),

    warning: useCallback((message: string) => { enhancedUserNotificationSystem.warning(message, {
        position: notificationPosition 
})
}, [notificationPosition]),

    info: useCallback((message: string) => { enhancedUserNotificationSystem.info(message, {
        position: notificationPosition 
})
}, [notificationPosition]),

    loading: useCallback((message: string) => { enhancedUserNotificationSystem.loading(message, {
        position: notificationPosition 
})
}, [notificationPosition]) };

  // 清理错误状态
  const clearError = useCallback(() => { if (mountedRef.current) {
      setCurrentError(null)
}
  }, []);

  // 清理所有错误
  const clearAllErrors = useCallback(() => { if (mountedRef.current) {
      setCurrentError(null);
      setErrorHistory([])
}
  ...
  // 获取错误恢复建议
  const getRecoverySuggestion = useCallback((error?: UnifiedErrorData): string => { const targetError = error || currentError;
  ...
  // 错误边界处理
  useEffect(() => { if (!enableErrorBoundary) return;

/**
 * handleUnhandledRejection函数
 * 处理事件
 *
 * @param event - event参数
 * @returns void
 ...
 * @returns void
 */
const handleError = (event: ErrorEvent) => { handleError(event.error,  {
...
* 专门用于React错误边界的错误处理
 */
export function useErrorBoundary(componentName?: string) { const { handleError  
} = useUnifiedErrorHandling( { componentName: componentName || 'ErrorBoundary',
    autoNotify: true,
...
* 专门用于处理异步操作中的错误
 */
export function useAsyncErrorHandling(componentName?: string) { const { handleError, wrapAsync, retry  } = useUnifiedErrorHandling( { componentName: componentName || 'AsyncOperation',
    autoNotify: true,
    recoveryStrategy: 'notify',
    maxRetries: 3 
});
...
* 专门用于表单验证和提交错误处理
 */
export function useFormErrorHandling(formName?: string) { const { handleError, notify  } = useUnifiedErrorHandling( { componentName: formName || 'Form',
  ...
  const handleValidationError = useCallback((;
    ...
    // 显示通用错误
    if (generalError) { notify.error(generalError)
}

    // 记录验证错误
    handleError(new Error('表单验证失败'), { action: 'form_validation',
      metadata: { fieldErrors, generalError  } })
}, [handleError, notify]);

  const handleSubmitError = useCallback((error: any) => { handleError(error, {
      action: 'form_submit',
      metadata: { formName  
} })
