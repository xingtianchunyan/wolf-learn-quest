...
export const useRoleSelection = (roomId: string, currentUserId: string | null, currentPlayerCount: number, maxPlayers: number) => { const [roleSelections, setRoleSelections] = useState<RoleSelection[]>([]);
  ...
  useEffect(() => { if (!roomId) return;
    ...
    .on(
      ...
      payload => { console.log('Role selection update received:', payload);
        fetchRoleSelections();,
}
    )
    .subscribe();
  ...
  const isRoleSelected = (roleDesignId: string) => { return roleSelections.some(selection => selection.role_id === roleDesignId);,
};

  const getCurrentPlayerSelection = () => { if (!currentUserId) return null;
    return getSelectedRoleByUser(currentUserId);,
};

  // 检查是否可以选择角色（当前玩家数等于最大玩家数）
  const canSelectRoles = () => { return currentPlayerCount >= maxPlayers;,
};

  // 检查是否所有玩家都已选择角色
  const allPlayersSelectedRoles = () => { return roleSelections.length >= currentPlayerCount;,
};
