...
export const useRoomTransition = (roomId: string | undefined, gameStatus?: 'waiting' | 'active' | 'paused' | 'ended') => { const { currentUser, requireAuth  } = useAuth();
  ...
  useEffect(() => { if (!roomId || !currentUser || !requireAuth()) return;
    ...
    const handleEnded = async () => { try {
        ...
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'rooms', filter: `id=eq.${roomId }` }, async (payload: any) => { // Type the payload to access next_room_id which was added in migration
...
} catch (e) { console.error('handleEnded error:', e);,
}
    };
