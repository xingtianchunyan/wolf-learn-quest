...
export const useAutoProcessDayVote = (;
  ...
  useEffect(() => { const run = async () => {
      ...
      try { // 1) Find the voting session for the just-ended day (phase 1)
        ...
        const { error: calcErr  } = await supabase.rpc('calculate_voting_results', { p_voting_session_id: session.id,
         });
        ...
        for (const r of results) { if (r.processing_status !== 'completed') {
            const { error: procErr  } = await supabase.rpc('process_voting_result', { p_voting_result_id: r.id,
             });
