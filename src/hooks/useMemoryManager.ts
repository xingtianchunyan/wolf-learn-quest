...
interface MemoryManagerOptions { componentName: string;
  maxMemoryThreshold?: number; // MB
  checkInterval?: number; // 毫秒
  enableAutoCleanup?: boolean;,
}

export const useMemoryManager = (options: MemoryManagerOptions) => { const {
    componentName,
  ...
  // 注册间隔任务
  const registerInterval = useCallback((intervalId: NodeJS.Timeout) => { intervalRefs.current.add(intervalId);
  ...
  // 注册超时任务
  const registerTimeout = useCallback((timeoutId: NodeJS.Timeout) => { timeoutRefs.current.add(timeoutId);
  ...
  // 注册订阅
  const registerSubscription = useCallback((unsubscribeFn: () => void) => { subscriptionsRef.current.add(unsubscribeFn);
  ...
  // 内存检查函数
  const checkMemoryUsage = useCallback(() => { if ('memory' in performance) {
      ...
      if (usedMB > maxMemoryThreshold) {
        logger.warn(`${componentName }内存使用超过阈值`, { current: `${usedMB.toFixed(2) }MB`,
          threshold: `${ maxMemoryThreshold }MB`,
          total: `${ (memory.totalJSHeapSize / (1024 * 1024)).toFixed(2) }MB`,
});

        // 触发垃圾回收（如果可用）
        if (enableAutoCleanup && (globalThis as any)?.gc) { try {
            (globalThis as any).gc();
            logger.debug(`${componentName }触发垃圾回收`);,
} catch (error) { logger.debug('垃圾回收不可用');,
}
        }

        return { isOverThreshold: true,
          currentUsage: usedMB,
  ...
  // 立即清理所有资源
  const forceCleanup = useCallback(() => { // 清理所有间隔任务
    ...
    // 取消所有订阅
    subscriptionsRef.current.forEach(unsubscribe => { try {
        unsubscribe();,
} catch (error) { logger.error(`${componentName }取消订阅失败`, error);,
}
    });
    subscriptionsRef.current.clear();

    // 执行所有清理函数
    cleanupFunctionsRef.current.forEach(cleanup => { try {
        cleanup();,
} catch (error) { logger.error(`${componentName }清理函数执行失败`, error);,
}
    });
  ...
  // 获取资源使用统计
  const getResourceStats = useCallback(() => { return {
  ...
  const createSafeAsync = useCallback(<T extends any[], R>(;
    ...
    return async (...args: T): Promise<R | null> => { if (!isMounted) {
        logger.debug(`${componentName }异步操作被取消：组件已卸载`);
        return null;,
}

      try { const result = await asyncFn(...args);
  ...
  // 定期内存检查
  useEffect(() => { if (!enableAutoCleanup) return;
  ...
  // 页面隐藏时的清理（可选）
  useEffect(() => { const handleVisibilityChange = () => {
