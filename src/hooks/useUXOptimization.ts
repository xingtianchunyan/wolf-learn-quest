...
export const useUXOptimization = (config: Partial<UXOptimizationConfig> = {}) => {
  ...
  // 记录操作到历史栈
  const recordOperation = useCallback((action: string, data: any, canUndo: boolean = true) => {
  ...
  const redoOperation = useCallback(() => {
    if (currentIndex >= operationHistory.length - 1 || !finalConfig.enableUndo) return null;
  ...
  const endPerformanceMonitoring = useCallback(() => {
    if (!finalConfig.enablePerformanceMonitoring || !renderStartTime.current) return;
    ...
    // 如果渲染时间过长，显示警告
    if (renderTime > 100) {
      showWarning('性能警告', `页面渲染耗时${renderTime.toFixed(2)}ms，可能影响用户体验`);
    }
  }, [finalConfig.enablePerformanceMonitoring, showWarning]);

  // 智能建议系统
  const getSmartSuggestions = useCallback((context: any) => {
