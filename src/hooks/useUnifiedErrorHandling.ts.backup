...
import { enhancedUserNotificationSystem,
import { ErrorSeverity  } from '@/utils/unifiedErrorHandler';
import { unifiedErrorManager,
...
export function useUnifiedErrorHandling(
  ...
  // 初始化时更新统计
  useEffect(() => { updateErrorStats();,
}, [updateErrorStats]);

  // 定期更新错误统计
  useEffect(() => { const interval = setInterval(updateErrorStats, 30000); // 每30秒更新一次
...
* 专门用于React错误边界的错误处理
 */
export function useErrorBoundary(componentName?: string) { const { handleError  } = useUnifiedErrorHandling({ componentName: componentName || 'ErrorBoundary',
    autoNotify: true,
...
export function useAsyncErrorHandling(componentName?: string) { const { handleError, wrapAsync, retry  } = useUnifiedErrorHandling({ componentName: componentName || 'AsyncOperation',
  ...
  const executeAsync = useCallback(async <T>(;
    operation: () => Promise<T>,
    context?: Partial<ErrorContext>
  ): Promise<T | null> => { const wrappedOperation = wrapAsync(operation, context);
  ...
  const executeWithRetry = useCallback(async <T>(;
    operation: () => Promise<T>,
    retryOptions?: { maxRetries?: number; delay?: number  }
  ): Promise<T | null> => { return await retry(operation, retryOptions);,
}, [retry]);
