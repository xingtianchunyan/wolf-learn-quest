...
export const useSkillStats = (;
  ...
  // 解决技能冲突
  const resolveSkillConflicts = useCallback(async (roundNumber: number) => { if (!gameStateId) return { resolved: 0, cancelled: 0  };
  ...
  // 检查是否有活跃效果
  const hasActiveEffect = useCallback((targetUserId: string, effectType: string) => { const targets = getUserSkillData(targetUserId).targets;
