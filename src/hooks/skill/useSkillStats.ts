...
export interface SkillSystemStats { totalUses: number;
...
export const useSkillStats = (
  ...
  // 计算统计数据
  const stats = useMemo((): SkillSystemStats => { return {
  ...
  // 获取用户相关的技能数据
  const getUserSkillData = useCallback((targetUserId?: string) => { const queryUserId = targetUserId || userId;
    if (!queryUserId) return {
    uses: [], effects: [], targets: []   
};

    return { // 只显示用户自己使用的技能记录，不显示其他玩家对自己使用技能的记录
  ...
  // 解决技能冲突
  const resolveSkillConflicts = useCallback(async (roundNumber: number) => { if (!gameStateId) return {
    resolved: 0, cancelled: 0   
};

    try { const result = await EnhancedSkillService.resolveSkillConflictsInRound(;
        gameStateId,
        roundNumber
      );
      ...
      toast({
        title: '技能冲突解决完成',
        description: `解决了 ${result.resolved 
} 个技能，取消了 ${ result.cancelled } 个冲突技能` });

      return result
} catch (error) { logger.error('解决技能冲突失败', error);
      toast({
        title: '冲突解决失败',
  ...
  // 检查是否有活跃效果
  const hasActiveEffect = useCallback((targetUserId: string, effectType: string) => {
