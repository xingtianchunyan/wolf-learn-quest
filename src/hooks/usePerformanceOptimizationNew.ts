...
interface PerformanceMetrics { renderCount: number;
...
export const usePerformanceOptimization = (options: UsePerformanceOptimizationOptions) => { const {
  ...
  // 渲染完成时间记录
  useEffect(() => { if (enableRenderTracking && startTimeRef.current > 0) {
    ...
    // 过度渲染警告
    if (metricsRef.current.renderCount > maxRenderThreshold) { const timeWindow = 1000; // 1秒窗口
...
): T => { const memoizedCallback = useCallback(callback, deps);
...
// 重置指标
const resetMetrics = useCallback(() => { metricsRef.current = {
    renderCount: 0,
    lastRenderTime: 0,
    averageRenderTime: 0,
    componentName,
