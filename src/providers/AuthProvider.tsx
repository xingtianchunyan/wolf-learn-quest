...
interface AuthContextType { currentUser: (User & { player_name?: string  }) | null;
  isLoggedIn: boolean;
...
export const AuthProvider: React.FC<{ children: React.ReactNode  }> = ({ children  }) => { const [currentUser, setCurrentUser] = useState<(User & { player_name?: string  }) | null>(null);
  ...
  useEffect(() => { // Listen for auth changes
    const { data: { subscription  } } = supabase.auth.onAuthStateChange(async (event, session) => { console.log('Auth change:', event, session?.user?.id);
