...
* 角色状态枚举
 */
export enum RoleStatus { NORMAL = 1,
  DYING = 2,
  WEAK = 3,
  ELIMINATED = 4;,
}
...
* 技能使用记录接口
 */
export interface SkillUsageRecord { /** 技能ID  */
  skillId: string;
  /** 已使用次数  */
  used: number;
  /** 剩余使用次数  */
...
* 角色技能使用状态接口
 */
export interface RoleSkillUsageState { /** 技能使用记录映射  */
...
* 回合技能使用记录接口
 */
export interface RoundSkillUsage { /** 回合号到技能ID列表的映射  */
...
* 角色状态接口
 */
export interface RoleState { /** 用户ID  */
  user_id: string;
  /** 游戏状态ID  */
...
* 旧版技能使用状态（向后兼容）
 */
export interface LegacySkillUsageState { /** 总使用次数  */
  total: number;
  /** 剩余使用次数  */
  remaining: number;,
}
...
* 角色设计接口
 */
export interface RoleDesign { /** 角色ID  */
  id: string;
  /** 角色名称  */
  role_name: string;
  /** 技能名称（英文）  */
...
* 技能配置接口
 */
export interface SkillConfig { /** 技能唯一标识  */
  id: string;
  /** 中文名称  */
  chineseName: string;
  /** 英文名称  */
...
* 技能使用上下文接口
 */
export interface SkillUsageContext { /** 用户ID  */
  userId: string;
  /** 游戏状态ID  */
  gameStateId: string;
  /** 房间ID  */
...
* 技能额外数据接口
 */
export interface SkillAdditionalData { /** 魔药类型（女巫专用）  */
...
* 技能验证结果接口
 */
export interface SkillValidationResult { /** 是否有效  */
  isValid: boolean;
  /** 失败原因  */
  reason?: string;
  /** 冲突的技能列表  */
...
* 技能验证详情接口
 */
export interface SkillValidationDetails { /** 阶段检查结果  */
  phaseCheck: boolean;
  /** 状态检查结果  */
  statusCheck: boolean;
  /** 使用次数检查结果  */
...
* 技能使用数据库记录接口
 */
export interface SkillUseRecord { /** 记录ID  */
  id: string;
  /** 游戏状态ID  */
  game_state_id: string;
  /** 用户ID  */
...
* 技能效果数据接口
 */
export interface SkillEffectData { /** 效果类型  */
...
* 技能冲突检测结果接口
 */
export interface SkillConflictResult { /** 是否有冲突  */
...
* 技能冲突详情接口
 */
export interface SkillConflictDetail { /** 技能1  */
  skill1: SkillConfig;
  /** 技能2  */
  skill2: SkillConfig;
  /** 冲突类型  */
...
* 技能使用建议接口
 */
export interface SkillUsageSuggestion { /** 是否可以使用  */
  canUse: boolean;
  /** 建议内容  */
  suggestion: string;
  /** 优先级  */
...
* 技能统计信息接口
 */
export interface SkillStatistics { /** 总使用次数  */
  totalUses: number;
  /** 成功次数  */
  successCount: number;
  /** 失败次数  */
...
* 技能错误接口
 */
export interface SkillError { /** 错误类型  */
  type: SkillErrorType;
  /** 错误代码  */
  code: string;
  /** 错误消息  */
...
): state is RoleSkillUsageState { return typeof state === 'object' &&;
  state !== null &&;
...
export function isLegacySkillUsageState(
  state: RoleSkillUsageState | LegacySkillUsageState
): state is LegacySkillUsageState { return typeof state === 'object' &&;
  state !== null &&;
  'total' in state &&
  'remaining' in state;,
}
...
* 类型守卫：检查是否为有效的角色状态
 */
export function isValidRoleStatus(status: unknown): status is RoleStatus { return typeof status === 'number' &&;
...
* 类型守卫：检查是否为有效的游戏阶段
 */
export function isValidGamePhase(phase: unknown): phase is GamePhase { return typeof phase === 'string' &&;
...
* 默认技能配置映射
 */
export const DEFAULT_SKILL_CONFIGS: Record<string, SkillConfig> = { 'Sleep': {
    id: 'villager_sleep',
    chineseName: '睡觉',
    englishName: 'Sleep',
    priority: 1,
  ...
  'night_attack': { id: 'werewolf_attack',
    chineseName: '夜袭',
