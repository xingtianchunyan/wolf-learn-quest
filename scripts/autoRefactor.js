...
class AutoRefactor {
  ...
  async run(options = {}) {
    ...
    console.log(`✨ 自动重构完成！${this.dryRun ? ' (试运行模式，未实际修改文件)' : ''}`);
  }
  ...
  async removeUnusedImports(content, filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (importMatch) {
        imports.push({
          lineIndex: i,
          fullLine: line,
          namedImports: importMatch[1] ? importMatch[1].split(',').map(s => s.trim()) : [],
    ...
    for (let i = imports.length - 1; i >= 0; i--) {
      ...
      // 检查命名导入
      const usedNamedImports = imp.namedImports.filter(name => usedIdentifiers.has(name));
  ...
  async extractConstants(content, filePath) {
    ...
    while ((match = magicNumbers.exec(content)) !== null) {
      ...
      if (number !== '100' && number !== '200' && number !== '404') { // 排除常见的HTTP状态码
    ...
    if (constants.length > 0) {
      ...
      lines.splice(importEndIndex + 1, 0, '', '// 提取的常量', ...constants, '');
  ...
  async simplifyConditionals(content, filePath) {
    ...
    modifiedContent = modifiedContent.replace(/if\s*\(\s*([^)]+)\s*===\s*true\s*\)/g, 'if ($1)');
    
    // 简化 if (condition === false) 为 if (!condition)
    modifiedContent = modifiedContent.replace(/if\s*\(\s*([^)]+)\s*===\s*false\s*\)/g, 'if (!$1)');
    ...
    modifiedContent = modifiedContent.replace(/([^?]+)\s*\?\s*true\s*:\s*false/g, '$1');
    
    // 简化三元运算符 condition ? false : true 为 !condition
    modifiedContent = modifiedContent.replace(/([^?]+)\s*\?\s*false\s*:\s*true/g, '!($1)');
  ...
  async removeDeadCode(content, filePath) {
    ...
    modifiedContent = modifiedContent.replace(/^\s*console\.log\([^)]*\);\s*$/gm, '');
    ...
    const filteredLines = lines.filter(line => {
      ...
      if (trimmed.startsWith('//')) {
        // 保留文档注释和说明性注释
        return !trimmed.match(/\/\/\s*(const|let|var|function|if|for|while|return)/);
      }
      return true;
    });
  ...
  async extractFunctions(content, filePath) {
    ...
    if (extractedFunctions.length > 0) {
      const importEndIndex = this.findImportEndIndex(lines);
      lines.splice(importEndIndex + 1, 0, '', '// 提取的函数', ...extractedFunctions);
      modifiedContent = lines.join('\n');
    }
...
${Object.entries(report.summary.rulesSummary).map(([rule, count]) => `
- **${rule}**: ${count} 个文件
`).join('')}
...
// 执行脚本
if (import.meta.url.endsWith(process.argv[1]) || process.argv[1].endsWith('autoRefactor.js')) {
