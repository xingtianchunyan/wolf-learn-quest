...
BEGIN
    ...
    IF p_potion_type = 'protection' THEN
        ...
        IF night_deaths IS NULL OR jsonb_array_length(night_deaths) = 0 THEN
            RETURN json_build_object(
                'can_use', false,
                'reason', '当夜没有死亡信息，无法使用解药',
                'suggested_action', '等待夜晚行动结束后再决定'
            );
        END IF;
...
-- 4. 创建技能执行优先级队列处理函数
CREATE OR REPLACE FUNCTION public.process_skill_execution_queue(
    p_game_state_id UUID,
    p_round_number INTEGER,
    p_phase TEXT
)
...
BEGIN
    ...
    FOR skill_record IN
        ...
        ORDER BY skill_priority ASC, created_at ASC
    LOOP
        BEGIN
            -- 根据技能类型处理不同的逻辑
            CASE skill_record.skill_name
                WHEN 'night_attack' THEN
                    PERFORM public.process_werewolf_attack(skill_record.id);
                WHEN 'vigil' THEN
                    PERFORM public.process_guard_protection(skill_record.id);
                WHEN 'prophecy' THEN
                    PERFORM public.process_seer_investigation(skill_record.id);
                WHEN 'magic_potion' THEN
                    PERFORM public.process_witch_potion(skill_record.id);
                WHEN 'self_destruct' THEN
                    PERFORM public.process_white_wolf_self_destruct(skill_record.id);
                WHEN 'dying_shot' THEN
                    PERFORM public.process_hunter_revenge(skill_record.id);
                ELSE
                    -- 通用技能处理
                    PERFORM public.apply_generic_effect(skill_record.id);
            END CASE;
    ...
    result_data := json_build_object(
        'processed_skills', processed_count,
